  ...
  return (
    <article className="seo-provider-card">
      ...
      <div className="seo-provider-copy">
        ...
        <div className="seo-provider-metrics" aria-label="Provider profile summary"><span><i>♥</i><b>{provider.likes}</b><small>{provider.likes===1?"Like":"Likes"}</small></span><span><i>★</i><b>{provider.reviews>0?provider.rating.toFixed(1):"New"}</b><small>{provider.reviews>0?`${provider.reviews} reviews`:"No ratings"}</small></span><span><i>↗</i><b>{provider.requestsReceived}</b><small>{provider.requestsReceived===1?"Request received":"Requests received"}</small></span></div>
