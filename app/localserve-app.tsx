...
function RepliedProviderTile({request,reply,updating,onSelect,onStart,onMessages}:{request:ServiceRequest;reply:RequestReply;updating:boolean;onSelect:()=>void;onStart:()=>void;onMessages:()=>void}){
...
function CleanMessagesView({user,onFind,onRequests,initialRequestId="",initialProviderId=""}:{user:SessionUser;onFind:()=>void;onRequests:()=>void;initialRequestId?:string;initialProviderId?:string}) {
  const [conversations,setConversations]=useState<Conversation[]>([]);const [selectedId,setSelectedId]=useState("__first__");const [message,setMessage]=useState("");const [loading,setLoading]=useState(true);const [sending,setSending]=useState(false);const [lifecycleBusy,setLifecycleBusy]=useState(false);const [error,setError]=useState("");
  ...
  async function updateLifecycle(action:"accept"|"start"){if(!selected)return;setLifecycleBusy(true);setError("");try{const response=await fetch(`/api/requests/${selected.requestId}`,{method:"PATCH",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({action,providerId:selected.providerId})});const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to update the work status");const assignedProviderId=String(result.data.assignedProviderId||selected.assignedProviderId||"");setConversations(current=>current.map(item=>item.requestId===selected.requestId?{...item,status:result.data.status,assignedProviderId,isSelected:item.providerId===assignedProviderId}:item))}catch(problem){setError(problem instanceof Error?problem.message:"Unable to update the work status")}finally{setLifecycleBusy(false)}}
...
function CustomerDashboard({user,savedProviders,onProvider,onRequest,onRequests,onMessages}:{user:SessionUser;savedProviders:Provider[];onProvider:(provider:Provider)=>void;onRequest:()=>void;onRequests:()=>void;onMessages:(requestId:string,providerId?:string)=>void}){
  ...
  const activeRequests=requests.filter(item=>["open","quoted","accepted","in_progress"].includes(item.status));
  ...
  const upcoming=requests.filter(item=>["accepted","in_progress"].includes(item.status));
  ...
  const detailConfig:Record<CustomerDetail,{title:string;subtitle:string;items:ServiceRequest[]}>= {active:{title:"Active requests",subtitle:"Requests currently waiting, selected or underway.",items:activeRequests},quotes:{title:"Quotations received",subtitle:"Professional replies and estimates for your requests.",items:quotedRequests},bookings:{title:"Upcoming bookings",subtitle:"Selected professionals and work currently in progress.",items:upcoming},saved:{title:"Saved providers",subtitle:"Professionals you saved for quick access.",items:[]},expenses:{title:"Completed service expenses",subtitle:`Total recorded spending ₹${totalExpenses.toLocaleString("en-IN")}.`,items:completed}};
...
function ProviderDashboard({role,user,onAction,onRequest,onSetup,onView,onMessages}:{role:SessionUser["role"];user:SessionUser;onAction:(s:string)=>void;onRequest:()=>void;onSetup:()=>void;onView:()=>void;onMessages:(requestId:string)=>void}) {
  ...
  useEffect(()=>{
    ...
    const hasActive=enquiries.some(item=>item.status!=="completed"&&(Boolean(item.responses?.length)||["accepted","in_progress"].includes(item.status)));
  ...
  const activeCustomers=enquiries.filter(item=>item.status!=="completed"&&(Boolean(item.responses?.length)||["accepted","in_progress"].includes(item.status)));
  const activeJobs=enquiries.filter(item=>["accepted","in_progress"].includes(item.status));
