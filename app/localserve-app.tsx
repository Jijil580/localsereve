  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {view === "dashboard" && currentUser && (currentUser.role==="customer"?<CustomerDashboard user={currentUser} savedCount={saved.length} onRequest={openRequest} onRequests={()=>navigate("requests")} onMessages={(requestId)=>{setMessageRequestId(requestId);navigate("messages")}}/>:<ProviderDashboard role={currentUser.role} user={currentUser} onAction={notify} onRequest={openRequest} onSetup={()=>setModal("profile")} onView={()=>setModal("profile-view")} onMessages={(requestId)=>{setMessageRequestId(requestId);navigate("messages")}} />)}
      </main>
...
function CustomerDashboard({user,savedCount,onRequest,onRequests,onMessages}:{user:SessionUser;savedCount:number;onRequest:()=>void;onRequests:()=>void;onMessages:(requestId:string)=>void}){
  ...
  const totalExpenses=requests.filter(item=>item.status==="completed").reduce((total,item)=>total+Number(item.finalAmount||0),0);
  const firstName=user.fullName.trim().split(/\s+/)[0]||"there";
  const stats=[[String(activeRequests.length),"Active requests"],[String(quotesReceived),"Quotes received"],[String(upcoming.length),"Upcoming bookings"],[String(savedCount),"Saved providers"],[`₹${totalExpenses.toLocaleString("en-IN")}`,"Total service expenses"]];
...
function ProviderDashboard({role,user,onAction,onRequest,onSetup,onView,onMessages}:{role:SessionUser["role"];user:SessionUser;onAction:(s:string)=>void;onRequest:()=>void;onSetup:()=>void;onView:()=>void;onMessages:(requestId:string)=>void}) {
  ...
  return <div className="dash-page">
    ...
    <div className="stat-grid">{stats.map(([value,label],index)=><div className="stat-card" key={label}><i>{["↗","◫","₹","✓"][index]}</i><b>{value}</b><span>{label}</span><small>{index===0&&newRequests.length?"Waiting for your first reply":index===1&&activeCustomers.length?"Tap a customer to chat":index>1&&completed.length?"From completed Nearleo work":"No activity yet"}</small></div>)}</div>
