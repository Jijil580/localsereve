...
type Conversation={id:string;requestId:string;providerId:string;providerPhotoUrl:string;requestNumber:string;service:string;status:string;isSelected:boolean;assignedProviderId:string;customerName:string;providerName:string;quoteAmount:number;finalAmount:number;confirmedAt?:string|null;startedAt?:string|null;completedAt?:string|null;availability:string;messages:ConversationMessage[];lastMessage:string;updatedAt:string;unread:number};
type JobTracking={status:string;address:string;targetLocation:MapLocation|null;sharing:boolean;providerLocation:(MapLocation&{accuracy?:number|null;updatedAt?:string|null})|null};
...
function CleanMessagesView({user,onFind,onRequests,initialRequestId="",initialProviderId=""}:{user:SessionUser;onFind:()=>void;onRequests:()=>void;initialRequestId?:string;initialProviderId?:string}) {
  ...
  async function updateLifecycle(action:"accept"|"confirm"|"start"|"complete"){if(!selected)return;if((action==="start"||action==="complete")&&user.role!=="provider"){setError("The selected provider will update this work status.");return}setLifecycleBusy(true);setError("");try{const finalAmount=action==="complete"?Number(completionAmount):undefined;if(action==="complete"&&(completionAmount===""||!Number.isFinite(finalAmount)||Number(finalAmount)<0||Number(finalAmount)>10000000)){setError("Enter a valid final amount to complete this job.");return}const response=await fetch(`/api/requests/${selected.requestId}`,{method:"PATCH",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({action,providerId:selected.providerId,finalAmount})});const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to update the work status");const assignedProviderId=String(result.data.assignedProviderId||selected.assignedProviderId||"");setConversations(current=>current.map(item=>item.requestId===selected.requestId?{...item,status:result.data.status,assignedProviderId,isSelected:item.providerId===assignedProviderId,confirmedAt:result.data.confirmedAt??item.confirmedAt,startedAt:result.data.startedAt??item.startedAt,completedAt:result.data.completedAt??item.completedAt,finalAmount:Number(result.data.finalAmount??item.finalAmount)}:item))}catch(problem){setError(problem instanceof Error?problem.message:"Unable to update the work status")}finally{setLifecycleBusy(false)}}
...
function ProviderDashboard({role,user,onAction,onRequest,onSetup,onView,onMessages}:{role:SessionUser["role"];user:SessionUser;onAction:(s:string)=>void;onRequest:()=>void;onSetup:()=>void;onView:()=>void;onMessages:(requestId:string)=>void}) {
  ...
  useEffect(()=>{
    ...
    const hasActive=enquiries.some(item=>item.status!=="completed"&&(Boolean(item.responses?.length)||["accepted","confirmed","in_progress"].includes(item.status)));
  ...
  const activeCustomers=enquiries.filter(item=>item.status!=="completed"&&(Boolean(item.responses?.length)||["accepted","confirmed","in_progress"].includes(item.status)));
