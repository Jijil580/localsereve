...
import WebRtcCallCenter, { type CallProvider } from "./webrtc-call";
...
type WhatsAppPermission = { _id:string;customerName:string;service:string;requestNumber?:string;status:"pending"|"approved"|"declined";requestedAt:string;respondedAt?:string };
type Provider = {
...
export default function NearleoApp() {
  ...
  const [requestedCallProvider,setRequestedCallProvider]=useState<CallProvider|null>(null);
  ...
  const results = useMemo(() => {
    ...
    const list = providers.filter(p => (!q || `${p.name} ${p.business} ${p.service} ${malayalamServiceNames[p.service]||""}`.toLowerCase().includes(q) || p.service===suggestedService) && (!q || p.distance === null || p.distance <= radius) && (!verifiedOnly || p.verified) && (!availableOnly || p.available));
  ...
  }, [providers, query, radius, verifiedOnly, availableOnly, sort,language]);
  ...
  function openContactSignIn(){setAuthMode("login");setPostAuthAction("contact");setModal("auth")}
  function requestVoiceCall(provider:Provider){if(!currentUser||currentUser.role!=="customer"){notify("Sign in with a customer account to call a professional.");return;}setSelected(null);setRequestedCallProvider({id:provider.id,business:provider.business,service:provider.service});}
  ...
  return (
    <div className="app-shell">
      ...
      {selected && !modal && <ProviderDrawer provider={selected} user={currentUser} saved={saved.includes(selected.id)} onClose={() => setSelected(null)} onBook={() => setModal("booking")} onCall={()=>requestVoiceCall(selected)} onSignIn={openContactSignIn} onSave={() => setSaved(s => s.includes(selected.id) ? s.filter(x=>x!==selected.id) : [...s,selected.id])} />}
...
function CallAccess({provider,user,onSignIn,onCall,compact=false}:{provider:Provider;user:SessionUser|null;onSignIn:()=>void;onCall:()=>void;compact?:boolean}) {
  const [status,setStatus]=useState<"none"|"pending"|"approved"|"declined"|"loading">("none");
  const [error,setError]=useState("");
  async function handleAction(){
    if(!user){onSignIn();return}
    if(user.role!=="customer"){setError("Use a customer account to request a call.");return}
    if(status==="approved"){onCall();return}
    setStatus("loading");setError("");
    try{
      const checking=status==="pending";
      const response=await fetch(`/api/providers/${provider.id}/call-request`,{method:checking?"GET":"POST",credentials:"include"});
      const result=await response.json();
      if(!response.ok)throw new Error(result.error||"Unable to update audio-call permission");
      setStatus(result.status||"pending");
    }catch(problem){setError(problem instanceof Error?problem.message:"Unable to update audio-call permission");setStatus(status==="loading"?"none":status)}
  }
  const label=!user?"Sign in to request call":status==="loading"?"Checking…":status==="approved"?"Start Audio Call":status==="pending"?"Check Call Approval":status==="declined"?"Request Call Again":"Request Audio Call";
  if(compact)return <div className="call-access compact"><button className="provider-quick-call" type="button" onClick={handleAction} disabled={status==="loading"} aria-label={`${label} with ${provider.business}`}><span>☎</span>{label}</button>{error&&<small>{error}</small>}</div>;
  return <div className={`call-access status-${status}`}><b>Private internet audio call</b><span>{status==="approved"?"Provider approved your request. You can now start an internet call.":status==="pending"?"Waiting for the provider to approve or decline your call request.":status==="declined"?"The provider declined this request. You may send a new request later.":"Send a permission request first. Phone numbers stay private."}</span><button className={status==="approved"?"voice-call-btn":""} type="button" onClick={handleAction} disabled={status==="loading"}>☎ {label}</button>{error&&<small>{error}</small>}</div>;
}

function ProviderDrawer({provider:p,user,saved,onClose,onBook,onCall,onSave,onSignIn}:{provider:Provider;user:SessionUser|null;saved:boolean;onClose:()=>void;onBook:()=>void;onCall:()=>void;onSave:()=>void;onSignIn:()=>void}) {
  return <div className="overlay" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><aside className="provider-drawer"><button className="modal-close" onClick={onClose}>×</button><div className="drawer-cover"><img src={p.cover} alt={`${p.business} profile`}/></div><div className="drawer-content"><div className="profile-main"><div className="avatar large">{p.image}</div><div><h2>{p.business} {p.verified&&<span className="verified">✓</span>}</h2><p>{p.name} · {p.service}</p><span className={`verification-badge ${p.verified?"approved":"unverified"}`}>{p.verified?"✓ VERIFIED":"UNVERIFIED"}</span></div></div><div className="profile-stats"><div><b>{p.reviews>0?`★ ${p.rating.toFixed(1)}`:"New"}</b><span>{p.reviews>0?`${p.reviews} verified reviews`:"No reviews yet"}</span></div><div><b>{p.jobs||"—"}</b><span>{p.jobs>0?"Completed jobs":"No jobs recorded"}</span></div><div><b>{p.experience>0?`${p.experience} yrs`:"—"}</b><span>Experience</span></div><div><b>{p.distance!==null?`${p.distance} km`:p.locality}</b><span>{p.distance!==null?"Distance":"Service area"}</span></div></div><div className={`profile-verification ${p.verified?"":"unverified"}`}><b>{p.verified?"✓ Identity and profile reviewed":"Unverified provider"}</b><span>{p.verified?"Nearleo admin reviewed this provider’s submitted identity and business information.":"This provider is listed but has not been verified by Nearleo. Confirm details before hiring."}</span></div><div className="contact-permission-panel"><CallAccess provider={p} user={user} onSignIn={onSignIn} onCall={onCall}/><WhatsAppAccess provider={p} user={user} onSignIn={onSignIn}/></div>{p.description&&<><h3>About</h3><p>{p.description}</p></>}<h3>Starting price</h3><div className="service-price"><span><b>{p.service}</b><small>Confirm scope, materials and final price directly with the provider.</small></span><b>{p.price>0?`From ₹${p.price}`:"Ask for estimate"}</b></div>{p.portfolio.length>0?<><h3>Recent work uploaded by provider</h3><div className="portfolio-row">{p.portfolio.map((image,index)=><img src={image} loading="lazy" alt={`${p.business} recent work ${index+1}`} key={image}/>)}</div></>:<div className="portfolio-empty"><b>No work gallery yet</b><span>This provider has not uploaded recent-work photos.</span></div>}<div className="drawer-actions"><button onClick={onSave}>{saved?"♥ Saved":"♡ Save"}</button><button className="primary-btn" onClick={onBook}>Request service</button></div></div></aside></div>
}

function WhatsAppAccess({provider,user,onSignIn}:{provider:Provider;user:SessionUser|null;onSignIn:()=>void}) {
  const [status,setStatus]=useState<"none"|"pending"|"approved"|"declined"|"loading">(user?.role==="customer"?"loading":"none");
  const [url,setUrl]=useState("");
  const [error,setError]=useState("");
  useEffect(()=>{if(user?.role!=="customer")return;fetch(`/api/providers/${provider.id}/whatsapp-request`,{credentials:"include"}).then(async response=>{const result=await response.json();if(!response.ok)throw new Error(result.error);setStatus(result.status||"none");setUrl(result.whatsappUrl||"")}).catch(problem=>{setError(problem instanceof Error?problem.message:"Unable to load WhatsApp access");setStatus("none")})},[provider.id,user?.id,user?.role]);
  async function requestAccess(){setStatus("loading");setError("");try{const response=await fetch(`/api/providers/${provider.id}/whatsapp-request`,{method:"POST",credentials:"include"});const result=await response.json();if(!response.ok)throw new Error(result.error);setStatus(result.status||"pending")}catch(problem){setError(problem instanceof Error?problem.message:"Unable to request WhatsApp contact");setStatus("none")}}
  if(!user)return <div className="whatsapp-access"><b>Private WhatsApp chat</b><span>Sign in as a customer to request permission from this provider.</span><button type="button" onClick={onSignIn}>Sign in to request WhatsApp chat</button></div>;
  if(user.role!=="customer")return <div className="whatsapp-access"><b>Private WhatsApp contact</b><span>WhatsApp access can be requested from a customer account.</span></div>;
  return <div className={`whatsapp-access status-${status}`}><b>Private WhatsApp chat</b>{status==="loading"?<span>Checking permission…</span>:status==="pending"?<span>Request Pending — the provider will approve or decline.</span>:status==="declined"?<><span>WhatsApp-chat request declined. You may request again later.</span><button type="button" onClick={requestAccess}>Request WhatsApp Chat Again</button></>:status==="approved"?<><span>WhatsApp Messaging Active</span><a className="whatsapp-btn" href={url} target="_blank" rel="noreferrer">Open WhatsApp Chat</a></>:<><span>The provider’s number stays private until they approve your request.</span><button type="button" onClick={requestAccess}>Request WhatsApp Chat</button></>}{error&&<small>{error}</small>}</div>;
}
...
function ProviderDashboard({role,user,onAction,onRequest,onSetup,onView}:{role:SessionUser["role"];user:SessionUser;onAction:(s:string)=>void;onRequest:()=>void;onSetup:()=>void;onView:()=>void}) {
  ...
  const [callRequests,setCallRequests]=useState<WhatsAppPermission[]>([]);
  const [whatsappRequests,setWhatsappRequests]=useState<WhatsAppPermission[]>([]);
  useEffect(()=>{
    ...
    fetch("/api/provider/call-requests",{credentials:"include"}).then(async response=>{const result=await response.json();if(response.ok)setCallRequests(result.data||[])}).catch(()=>{});
    fetch("/api/provider/whatsapp-requests",{credentials:"include"}).then(async response=>{const result=await response.json();if(response.ok)setWhatsappRequests(result.data||[])}).catch(()=>{});
  },[role]);
  async function updateCallRequest(id:string,action:"approve"|"decline"){const response=await fetch(`/api/provider/call-requests/${id}`,{method:"PATCH",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({action})});const result=await response.json();if(!response.ok){setEnquiryError(result.error||"Unable to update audio-call request");return}setCallRequests(current=>current.map(item=>item._id===id?{...item,status:result.status}:item))}
  async function updateWhatsAppRequest(id:string,action:"approve"|"decline"){const response=await fetch(`/api/provider/whatsapp-requests/${id}`,{method:"PATCH",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({action})});const result=await response.json();if(!response.ok){setEnquiryError(result.error||"Unable to update WhatsApp request");return}setWhatsappRequests(current=>current.map(item=>item._id===id?{...item,status:result.status}:item))}
  ...
  return <div className="dash-page"><div className="page-heading"><div><span className="kicker">PROVIDER DASHBOARD</span><h1>Welcome, {firstName}!</h1><p>View customer requests that match your listed service. Verification is not required.</p></div><div className="page-heading-actions"><button className="ghost-btn" onClick={onView}>View profile</button><button className="primary-btn" onClick={onSetup}>Set up profile</button></div></div><div className="stat-grid">{stats.map(([value,label],index)=><div className="stat-card" key={label}><i>{["↗","◫","₹","★"][index]}</i><b>{value}</b><span>{label}</span><small>{index<2&&enquiries.length?"Live customer requests":"No activity yet"}</small></div>)}</div><div className="dashboard-grid provider-dashboard-grid"><section className="panel provider-enquiries"><div className="panel-head"><h3>Requests and active jobs</h3><span>{enquiries.length} items</span></div>{loadingEnquiries?<div className="clean-empty compact"><h3>Loading enquiries…</h3></div>:enquiryError?<div className="auth-error">{enquiryError}</div>:!profileReady?<div className="clean-empty compact"><span>○</span><h3>Complete your provider profile</h3><p>Add the required service, location and contact details to receive matching requests.</p></div>:enquiries.length===0?<div className="clean-empty compact"><span>○</span><h3>No matching enquiries yet</h3><p>New customer requests for your service will appear here automatically.</p></div>:<div className="provider-enquiry-list">{enquiries.map(item=><ProviderEnquiryCard key={item._id} item={item} onUpdated={(id,status)=>setEnquiries(current=>status==="completed"?current.filter(entry=>entry._id!==id):current.map(entry=>entry._id===id?{...entry,quoteCount:status==="quoted"?entry.quoteCount+1:entry.quoteCount,status}:entry))}/>)}</div>}</section><section className="panel whatsapp-request-panel call-request-panel"><div className="panel-head"><h3>Audio call requests</h3><span>{callRequests.filter(item=>item.status==="pending").length} pending</span></div>{callRequests.length===0?<div className="clean-empty compact"><span>☎</span><h3>No audio-call requests</h3><p>Customers must ask for permission before they can call you.</p></div>:<div className="whatsapp-request-list">{callRequests.map(item=><article key={item._id}><div><b>{item.customerName}</b><span>{item.service}{item.requestNumber?` · ${item.requestNumber}`:""}</span><small>{new Date(item.requestedAt).toLocaleString()}</small></div><strong className={`status-${item.status}`}>{item.status==="approved"?"Calling active":item.status==="declined"?"Declined":"Pending"}</strong>{item.status==="pending"&&<div><button onClick={()=>updateCallRequest(item._id,"decline")}>Decline</button><button className="primary-btn" onClick={()=>updateCallRequest(item._id,"approve")}>Approve call</button></div>}</article>)}</div>}</section><section className="panel whatsapp-request-panel"><div className="panel-head"><h3>WhatsApp chat requests</h3><span>{whatsappRequests.filter(item=>item.status==="pending").length} pending</span></div>{whatsappRequests.length===0?<div className="clean-empty compact"><span>◌</span><h3>No WhatsApp-chat requests</h3><p>Customer permission requests will appear here.</p></div>:<div className="whatsapp-request-list">{whatsappRequests.map(item=><article key={item._id}><div><b>{item.customerName}</b><span>{item.service}{item.requestNumber?` · ${item.requestNumber}`:""}</span><small>{new Date(item.requestedAt).toLocaleString()}</small></div><strong className={`status-${item.status}`}>{item.status==="approved"?"Messaging active":item.status==="declined"?"Declined":"Pending"}</strong>{item.status==="pending"&&<div><button onClick={()=>updateWhatsAppRequest(item._id,"decline")}>Decline</button><button className="primary-btn" onClick={()=>updateWhatsAppRequest(item._id,"approve")}>Approve chat</button></div>}</article>)}</div>}</section><section className="panel"><div className="panel-head"><h3>Professional profile</h3><button onClick={onView}>View profile</button></div><div className="clean-empty compact"><span>＋</span><h3>Manage your profile</h3><p>Review and update the information customers see. Optional verification documents can be added any time.</p><button className="primary-btn" onClick={onSetup}>Edit profile</button></div></section></div></div>;
}
