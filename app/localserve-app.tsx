...
function RequestForm({onSuccess,onBeforeAction,provider,language,customerLocation}:{onSuccess:(message:string)=>void;onBeforeAction:(context:string,action:()=>void)=>void;provider?:Provider|null;language:Language;customerLocation?:MapLocation|null}) {
  ...
  return <form className="request-form" onSubmit={submit}>
    ...
    <div className="request-form-body">
      ...
      {step===3&&<div className="request-review">{provider&&<div><span>Preferred professional</span><b>{provider.business}</b></div>}<div><span>Service</span><b>{draft.service}</b></div><div><span>When</span><b>{draft.preferredDate} · {draft.preferredTime}</b></div><div className="request-review-address"><span>Address to use</span><b>{draft.address}</b><button type="button" onClick={()=>setStep(2)}>Edit address</button></div><div><span>Matching</span><b>{draft.location?`Near ${draft.location.label||"your selected location"}`:customerLocation?`Near ${customerLocation.label||"your location"}`:"By selected service"}</b></div><div><span>Urgency</span><b>{draft.urgency}</b></div>{draft.allowWhatsApp&&<div><span>WhatsApp</span><b>{draft.whatsappNumber}</b></div>}<p>{draft.description}</p><small>Continue only if the address above is correct. No booking or price is confirmed until you select and confirm a provider reply.</small></div>}
