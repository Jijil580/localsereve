...
type ServiceRequest = { _id:string;requestNumber:string;customerName?:string;service:string;description:string;address:string;location?:MapLocation|null;preferredDate:string;preferredTime:string;urgency:string;status:string;quoteCount:number;whatsappNumber?:string;responses?:RequestReply[];assignedProviderId?:string;assignedProviderName?:string;customerDistanceKm?:number|null;confirmedAt?:string;arrivedAt?:string;startedAt?:string;completedAt?:string;cancelledAt?:string;cancelledByRole?:string;finalAmount?:number;createdAt:string;updatedAt?:string };
...
type Conversation={id:string;requestId:string;providerId:string;providerPhotoUrl:string;requestNumber:string;service:string;status:string;isSelected:boolean;assignedProviderId:string;customerName:string;providerName:string;quoteAmount:number;finalAmount:number;confirmedAt?:string|null;arrivedAt?:string|null;startedAt?:string|null;completedAt?:string|null;cancelledAt?:string|null;cancelledByRole?:string;availability:string;messages:ConversationMessage[];lastMessage:string;updatedAt:string;unread:number};
...
function RepliedProviderTile({request,reply,updating,onSelect,onConfirm,onStart,onComplete,onMessages}:{request:ServiceRequest;reply:RequestReply;updating:boolean;onSelect:()=>void;onConfirm:()=>void;onStart:()=>void;onComplete:(finalAmount:number)=>void;onMessages:()=>void}){
  ...
  return <article className={`replied-provider-tile ${selected?"selected":""} ${anotherSelected?"not-selected":""}`}>
    ...
    <div className="reply-actions replied-provider-actions"><button type="button" className="ghost-btn" onClick={onMessages}>Open chat</button>{["open","quoted"].includes(request.status)&&<button type="button" className="primary-btn" disabled={updating} onClick={onSelect}>{updating?"Selecting…":"Mark as selected"}</button>}{selected&&request.status==="accepted"&&<button type="button" className="primary-btn confirm-btn" disabled={updating} onClick={onConfirm}>{updating?"Confirming…":"Confirm job"}</button>}{selected&&request.status==="confirmed"&&<button type="button" className="primary-btn progress-btn" disabled={updating} onClick={onStart}>{updating?"Updating…":"Mark work in progress"}</button>}{selected&&request.status==="in_progress"&&<><span className="provider-work-state">Work in progress</span><label className="reply-completion-amount"><span>Final amount (₹)</span><input type="number" min="0" max="10000000" inputMode="numeric" value={completionAmount} onChange={event=>setCompletionAmount(event.target.value)} placeholder="Amount paid"/></label><button type="button" className="primary-btn complete-btn" disabled={updating||completionAmount===""||!Number.isFinite(Number(completionAmount))||Number(completionAmount)<0} onClick={()=>onComplete(Number(completionAmount))}>{updating?"Completing…":"Mark completed"}</button></>}{selected&&request.status==="completed"&&<span className="provider-work-state completed">Job completed · ₹{Number(request.finalAmount||0).toLocaleString("en-IN")}</span>}{anotherSelected&&<span className="provider-work-state muted">Another provider selected</span>}</div>
  </article>
}
