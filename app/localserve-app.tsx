...
export default function LocalServeApp() {
  ...
  return (
    <div className="app-shell">
      ...
      <footer><div className="footer-brand"><div className="brand"><span className="brand-mark">L</span><span>Local<span>Serve</span></span></div><p>Trusted local services, one tap away.</p><small className="powered-by">Powered by <b>Lumier Technologies</b></small></div><div><b>Customers</b><a>Find services</a><a>Post a request</a><a>Safety</a></div><div><b>Professionals</b><a>Join LocalServe</a><a>Plans & pricing</a><a>Provider help</a></div><div><b>Company</b><a>About</a><a>Contact</a><a>Privacy & terms</a></div><div className="footer-bottom">© 2026 LocalServe. Made for local communities. <span>Powered by Lumier Technologies · English · हिन्दी · മലയാളം</span></div></footer>
...
function RequestsView({onPost}:{onPost:()=>void}) {
  ...
  return <div className="dash-page"><div className="page-heading"><div><span className="kicker">SERVICE REQUESTS</span><h1>My requests</h1><p>Your posted work, quotations and bookings appear here.</p></div><button className="primary-btn" onClick={onPost}>＋ Post a new request</button></div><div className="request-summary"><div><span>{active}</span>Active requests</div><div><span>{quotes}</span>Quotes received</div><div><span>0</span>Upcoming bookings</div></div>{loading?<div className="clean-empty"><h3>Loading your requests…</h3></div>:error?<div className="auth-error request-load-error">{error}</div>:requests.length===0?<div className="clean-empty"><span>＋</span><h3>No service requests yet</h3><p>Post your first request and nearby professionals will be able to send quotations.</p><button className="primary-btn" onClick={onPost}>Post your first request</button></div>:<div className="saved-request-list">{requests.map(item=><article className="saved-request-card" key={item._id}><div className="request-icon">{categories.find(category=>category[1]===item.service)?.[0]||"⌁"}</div><div><span className="status-pill amber">{item.status.toUpperCase()}</span><h3>{item.service}</h3><p>{item.description}</p><small>⌖ {item.address} · {item.preferredDate} · {item.preferredTime}</small></div><div className="saved-request-meta"><b>{item.requestNumber}</b><span>{item.quoteCount||0} quotes</span><small>{new Date(item.createdAt).toLocaleDateString()}</small></div></article>)}</div>}</div>;
}
...
function CleanDashboard({role,user,onAction,onRequest}:{role:SessionUser["role"];user:SessionUser;onAction:(s:string)=>void;onRequest:()=>void}) {
  ...
  const description = role === "provider"
    ...
    : role === "admin"
      ? "Platform activity will appear here as customers and professionals begin using LocalServe."
