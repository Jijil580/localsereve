  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {view === "dashboard" && !currentUser && <GuestAccountView language={language} onLogin={()=>{setAuthMode("login");setModal("auth")}} onRegister={()=>{setRole("customer");setAuthMode("register");setModal("auth")}} />}
      </main>
...
function GuestAccountView({language,onLogin,onRegister}:{language:Language;onLogin:()=>void;onRegister:()=>void}){
  const t=translations[language];
  return <section className="guest-account-page" aria-labelledby="guest-account-title"><div className="guest-account-card"><span className="guest-account-mark" aria-hidden="true">N</span><span className="kicker">{t.account}</span><h1 id="guest-account-title">{t.signInAccount}</h1><p>{t.registerDescription}</p><div className="guest-account-benefits"><span><MobileNavIcon name="requests"/><b>{t.requests}</b></span><span><MobileNavIcon name="messages"/><b>{t.messages}</b></span><span><MobileNavIcon name="account"/><b>{t.account}</b></span></div><div className="guest-account-actions"><button type="button" className="primary-btn" onClick={onLogin}>{t.signIn}</button><button type="button" onClick={onRegister}>{t.createAccount}</button></div></div></section>;
}
