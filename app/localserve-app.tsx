...
function OpeningIntro({onFinish}:{onFinish:()=>void}) {
  ...
  function nextChapter(){if(chapter>=5)onFinish();else setChapter(current=>current+1)}
  useEffect(()=>{const timer=window.setTimeout(nextChapter,10000);return()=>window.clearTimeout(timer)},[chapter]);
  return <section className="opening-intro" role="status" aria-label={`Nearlio introduction, chapter ${chapter+1} of 6`} onClick={event=>{if(!(event.target as HTMLElement).closest("button"))nextChapter()}}>
...
export default function NearlioApp() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setIntroVisible(false), 60000);
