...
export default function NearleoApp() {
  ...
  const results = useMemo(() => {
    ...
    const list = providers.filter(p => (!q || `${p.name} ${p.business} ${p.service} ${malayalamServiceNames[p.service]||""}`.toLowerCase().includes(q)) && (!q || p.distance === null || p.distance <= radius) && (!verifiedOnly || p.verified) && (!availableOnly || p.available));
