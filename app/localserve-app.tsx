...
type RequestReply = { providerId:string;providerName:string;providerBusiness:string;providerWhatsApp?:string;message:string;quoteAmount?:number;availability?:string;createdAt:string };
...
type Provider = {
  ...
  verified: boolean; image: string; cover: string; description: string; jobs: number; locality: string; whatsapp?:string; portfolio:string[];
};
...
function ServiceAutocomplete({value,onChange,onSelect,placeholder,ariaLabel,language="EN"}:{value:string;onChange:(value:string)=>void;onSelect?:(value:string)=>void;placeholder:string;ariaLabel:string;language?:Language}){
...
function OpeningIntro({onFinish}:{onFinish:()=>void}) {
  ...
  return <section className="opening-intro" role="status" aria-label={`LumNearo welcome, part ${chapter+1} of 3`} onClick={event=>{if(!(event.target as HTMLElement).closest("button"))nextChapter()}}>
    <div className="intro-chapters">
      <article className={`intro-chapter intro-chapter-1 ${chapter===0?"active":""}`}><img src="/near-lio-carpenter.jpg" alt="Carpenter completing skilled woodwork"/><div className="intro-chapter-shade"/><div className="intro-story intro-story-brand"><span className="intro-brand-mark">L</span><div><strong>Lum<span>Nearo</span></strong><small>by Lumier</small></div><p>Where Local Experts Meet Local Customers</p></div></article>
    ...
    <button type="button" className="intro-next" onClick={nextChapter}>{chapter===2?"Enter LumNearo":"Next →"}</button>
    <button type="button" className="intro-skip" onClick={finish}>Skip</button>
  </section>;
}

export default function LumNearoApp() {
  ...
  const results = useMemo(() => {
    ...
    const list = providers.filter(p => (!q || `${p.name} ${p.business} ${p.service} ${malayalamServiceNames[p.service]||""}`.toLowerCase().includes(q)) && (p.distance === null || p.distance <= radius) && (!verifiedOnly || p.verified) && (!availableOnly || p.available));
  ...
  async function signOut() {
    ...
    setAccountMenuOpen(false); setCurrentUser(null); setView("home"); notify("You have been signed out.");
  }
  ...
  return (
    <div className="app-shell">
      ...
      <header className="topbar">
        <button className="brand" onClick={() => setView("home")} aria-label="LumNearo by Lumier home"><span className="brand-mark">L</span><span className="brand-wordmark"><span>Lum<span>Nearo</span></span><small>by Lumier</small></span></button>
        ...
        <div className="header-actions">
          <button className="location-mini" onClick={useLocation}>⌖ <span>{customerLocation?.label||t.setLocation}</span></button>
          ...
          <section className="hero hero-reference">
            <div className="hero-copy">
              ...
              <form className="hero-search hero-search-expanded" onSubmit={e => {e.preventDefault(); goSearch()}}>
                ...
                <button type="button" className="location-field" onClick={useLocation}><span>⌖</span><span><small>{t.yourLocation}</small>{customerLocation?.label||t.location}</span></button>
                ...
                <button className="search-submit">{t.searchNow}</button>
              </form>
              <div className="hero-proof-row"><div><span>✓</span><b>{t.reviewedProfiles}</b></div><div><span>₹</span><b>{t.startingPrices}</b></div><div><span>⌖</span><b>{t.locationDiscovery}</b></div><div><span>↯</span><b>{t.savedReplies}</b></div></div>
            </div>
            <div className="hero-collage" aria-label="Verified LumNearo service professionals">
              ...
              <div className="happy-customers"><b>LumNearo</b><span>{t.builtForCommunities}</span></div>
            </div>
          </section>

          <div className="home-service-motion" aria-label="Popular LumNearo services"><div>{[...homeMotionServices,...homeMotionServices].map((service,index)=><span key={`${service}-${index}`}>✦ {serviceLabel(service,language)}</span>)}</div></div>
      ...
      <footer><div className="footer-brand"><div className="brand"><span className="brand-mark">L</span><span className="brand-wordmark"><span>Lum<span>Nearo</span></span><small>by Lumier</small></span></div><p>{t.tagline}</p><small className="powered-by">{t.poweredBy} <b>Lumier Technologies</b></small></div><div><b>{t.customers}</b><button onClick={() => goSearch()}>{t.findServices}</button><button onClick={openRequest}>{t.postRequest}</button><button onClick={() => setModal("safety")}>{t.safety}</button></div><div><b>{t.professionals}</b><button onClick={() => {setRole("provider");currentUser?setView("dashboard"):setModal("auth")}}>{t.joinNearlio}</button><button onClick={() => setModal("pricing")}>{t.plansPricing}</button><button onClick={() => setModal("provider-help")}>{t.providerHelp}</button></div><div><b>{t.company}</b><button onClick={() => setModal("about")}>{t.about}</button><button onClick={() => setModal("contact")}>{t.contact}</button><button onClick={() => setModal("terms")}>{t.privacyTerms}</button></div><div className="footer-bottom">© 2026 LumNearo. {t.tagline} <span>{t.poweredBy} Lumier Technologies · demo@lumiertechnologies.com</span></div></footer>
...
function RequestsView({onPost}:{onPost:()=>void}) {
  ...
  return <div className="dash-page"><div className="page-heading"><div><span className="kicker">SERVICE REQUESTS</span><h1>My requests</h1><p>Follow each request from posting to provider selection and completion.</p></div><button className="primary-btn" onClick={onPost}>＋ Post a new request</button></div><div className="request-summary"><div><span>{active}</span>Active requests</div><div><span>{quotes}</span>Replies received</div><div><span>{confirmed}</span>Confirmed jobs</div></div>{loading?<div className="clean-empty"><h3>Loading your requests…</h3></div>:error&&requests.length===0?<div className="auth-error request-load-error">{error}</div>:requests.length===0?<div className="clean-empty"><span>＋</span><h3>No service requests yet</h3><p>Post your first request and nearby professionals will be able to reply.</p><button className="primary-btn" onClick={onPost}>Post your first request</button></div>:<>{error&&<div className="auth-error request-load-error">{error}</div>}<div className="saved-request-list">{requests.map(item=>{const stepIndex=requestProgress.indexOf(item.status);return <article className="saved-request-card request-workflow-card" key={item._id}><div className="request-icon">{categories.find(category=>category[1]===item.service)?.[0]||"⌁"}</div><div className="request-main"><span className={`status-pill status-${item.status}`}>{requestStatusLabels[item.status]||item.status}</span><h3>{item.service}</h3><p>{item.description}</p><small>⌖ {item.address} · {item.preferredDate} · {item.preferredTime}</small>{item.status!=="cancelled"&&<div className="request-progress" aria-label={`Request status: ${requestStatusLabels[item.status]||item.status}`}>{requestProgress.map((status,index)=><span className={index<=stepIndex?"done":""} key={status}><i>{index<stepIndex?"✓":index+1}</i><small>{requestStatusLabels[status]}</small></span>)}</div>}{item.assignedProviderName&&<div className="selected-provider"><span>Selected professional</span><b>{item.assignedProviderName}</b></div>}{Boolean(item.responses?.length)&&<div className="customer-replies">{item.responses!.map((reply,index)=><div className={item.assignedProviderId===reply.providerId?"accepted-reply":""} key={`${reply.providerId}-${index}`}><div className="reply-head"><b>{reply.providerBusiness||reply.providerName}</b>{reply.quoteAmount!==undefined&&<strong>Estimate ₹{reply.quoteAmount.toLocaleString("en-IN")}</strong>}</div><p>{reply.message}</p>{reply.availability&&<span>Availability: {reply.availability}</span>}<small>{new Date(reply.createdAt).toLocaleString()}</small><div className="reply-actions">{reply.providerWhatsApp&&<a href={whatsappUrl(reply.providerWhatsApp,`Hello ${reply.providerName}, I am contacting you about ${item.requestNumber}.`)} target="_blank" rel="noreferrer">WhatsApp provider</a>}{["open","quoted"].includes(item.status)&&<button className="primary-btn" disabled={updatingId===item._id} onClick={()=>updateStatus(item,"accept",reply.providerId)}>Select provider</button>}</div></div>)}</div>}<div className="request-status-actions">{["open","quoted","accepted"].includes(item.status)&&<button disabled={updatingId===item._id} onClick={()=>updateStatus(item,"cancel")}>Cancel request</button>}{["accepted","in_progress"].includes(item.status)&&<button className="primary-btn" disabled={updatingId===item._id} onClick={()=>updateStatus(item,"complete")}>Mark job completed</button>}</div></div><div className="saved-request-meta"><b>{item.requestNumber}</b><span>{item.quoteCount||0} {item.quoteCount===1?"reply":"replies"}</span><small>{new Date(item.createdAt).toLocaleDateString()}</small></div></article>})}</div></>}</div>;
}



function ProviderDrawer({provider:p,saved,onClose,onBook,onSave}:{provider:Provider;saved:boolean;onClose:()=>void;onBook:()=>void;onSave:()=>void}) { return <div className="overlay" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><aside className="provider-drawer"><button className="modal-close" onClick={onClose}>×</button><div className="drawer-cover"><img src={p.cover} alt={`${p.business} profile`}/></div><div className="drawer-content"><div className="profile-main"><div className="avatar large">{p.image}</div><div><h2>{p.business} {p.verified&&<span className="verified">✓</span>}</h2><p>{p.name} · {p.service}</p><span className={`status-pill ${p.available?"green":"neutral"}`}>{p.available?"AVAILABLE TODAY":"CHECK AVAILABILITY"}</span></div></div><div className="profile-stats"><div><b>{p.reviews>0?`★ ${p.rating.toFixed(1)}`:"New"}</b><span>{p.reviews>0?`${p.reviews} verified reviews`:"No reviews yet"}</span></div><div><b>{p.jobs||"—"}</b><span>{p.jobs>0?"Completed jobs":"No jobs recorded"}</span></div><div><b>{p.experience>0?`${p.experience} yrs`:"—"}</b><span>Experience</span></div><div><b>{p.distance!==null?`${p.distance} km`:p.locality}</b><span>{p.distance!==null?"Distance":"Service area"}</span></div></div><div className="profile-verification"><b>✓ Identity and profile reviewed</b><span>LumNearo admin approved this provider’s submitted identity and business profile.</span></div><h3>About</h3><p>{p.description}</p><h3>Starting price</h3><div className="service-price"><span><b>{p.service}</b><small>Confirm scope, materials and final price directly with the provider.</small></span><b>{p.price>0?`From ₹${p.price}`:"Ask for estimate"}</b></div>{p.portfolio.length>0?<><h3>Recent work uploaded by provider</h3><div className="portfolio-row">{p.portfolio.map((image,index)=><img src={image} loading="lazy" alt={`${p.business} recent work ${index+1}`} key={image}/>)}</div></>:<div className="portfolio-empty"><b>No work gallery yet</b><span>This provider has not uploaded recent-work photos.</span></div>}<div className="drawer-actions"><button onClick={onSave}>{saved?"♥ Saved":"♡ Save"}</button>{p.whatsapp&&<a className="whatsapp-btn" href={whatsappUrl(p.whatsapp,`Hello ${p.name}, I found your ${p.service} profile on LumNearo.`)} target="_blank" rel="noreferrer">WhatsApp</a>}<button className="primary-btn" onClick={onBook}>Request service</button></div></div></aside></div> }

function InformationPanel({type,onFindServices,onJoinProvider}:{type:string;onFindServices:()=>void;onJoinProvider:()=>void}) {
  if(type==="pricing") return <div className="info-panel pricing-panel"><span className="kicker">SIMPLE PRICING</span><h2>Start free. Grow when you are ready.</h2><div className="launch-offer"><b>Launch offer: first 12 months free</b><span>Every LumNearo customer and professional can use their chosen plan free for the first year after joining.</span></div><div className="pricing-grid"><article><span>FOR CUSTOMERS</span><h3>Customer</h3><strong>₹0 <small>forever</small></strong><ul><li>Search nearby professionals</li><li>Post service requests</li><li>Receive replies and quotations</li><li>Save providers and use consent-based WhatsApp</li></ul><button className="primary-btn" onClick={onFindServices}>Find a service</button></article><article><span>FOR PROFESSIONALS</span><h3>Basic</h3><strong>₹0 <small>/ year</small></strong><ul><li>Verified public profile</li><li>Service location and nearby enquiries</li><li>Reply to customers</li><li>Basic profile photo and service information</li></ul><button className="primary-btn" onClick={onJoinProvider}>Join Basic</button></article><article className="featured-plan"><em>FIRST YEAR FREE</em><span>FOR PROFESSIONALS</span><h3>Growth</h3><strong>₹199 <small>/ year after year one</small></strong><ul><li>Everything in Basic</li><li>Work gallery and extra profile space</li><li>Enhanced business presentation</li><li>Priority growth support from Lumier</li></ul><a className="primary-btn" href="mailto:demo@lumiertechnologies.com?subject=LumNearo%20Growth%20Plan">Choose Growth</a></article></div><p className="fine-print">Taxes, billing activation and renewal reminders will be shown before any paid renewal. No automatic charge is made during the free launch year.</p></div>;
  const content:Record<string,{kicker:string;title:string;body:React.ReactNode}>={
    about:{kicker:"ABOUT LUMNEARO",title:"Local work. Local trust.",body:<><p>LumNearo is a local-services marketplace created by <b>Lumier Technologies</b>. We help customers discover nearby professionals and help skilled workers build a trusted digital presence.</p><div className="info-feature-grid"><div><b>Nearby discovery</b><span>Location-aware results connect customers with relevant professionals.</span></div><div><b>Verified profiles</b><span>Provider identity and profile details are reviewed before public approval.</span></div><div><b>Direct communication</b><span>Requests, replies and consent-based WhatsApp contact keep work moving.</span></div></div></>},
    contact:{kicker:"CONTACT",title:"Talk to Lumier Technologies",body:<><p>Questions about LumNearo, provider verification, plans, safety or partnerships are welcome.</p><div className="contact-card"><span>Company</span><b>Lumier Technologies</b><span>Email</span><a href="mailto:demo@lumiertechnologies.com">demo@lumiertechnologies.com</a><span>Service</span><b>LumNearo local-services marketplace</b></div><a className="primary-btn info-mail" href="mailto:demo@lumiertechnologies.com?subject=LumNearo%20Support">Email Lumier support</a></>},
    safety:{kicker:"TRUST & SAFETY",title:"Use LumNearo safely",body:<><p>Use contact details only for the genuine service request that made them available. Never send spam, repeated or unwanted messages, harassment, threats or unrelated promotions.</p><ul className="info-list"><li>Agree on scope, materials, price and timing before work begins.</li><li>Never share passwords, OTPs, PINs or banking credentials.</li><li>Keep quotations, receipts and important conversations in writing.</li><li>Stop contacting a user immediately when they withdraw permission.</li><li>Report suspicious accounts, fraud or unsafe conduct to Lumier support.</li></ul><a className="primary-btn info-mail" href="mailto:demo@lumiertechnologies.com?subject=LumNearo%20Safety%20Report">Report a safety concern</a></>},
    "provider-help":{kicker:"PROVIDER HELP",title:"Build a profile customers can trust",body:<><div className="help-steps"><div><span>1</span><b>Create a professional account</b><p>Add your name, phone number and primary service.</p></div><div><span>2</span><b>Complete verification</b><p>Upload a clear profile photo, identity card and accurate service location.</p></div><div><span>3</span><b>Receive nearby enquiries</b><p>Approved providers can view matching requests and reply inside LumNearo or through permitted WhatsApp contact.</p></div><div><span>4</span><b>Grow your profile</b><p>Keep your availability and details updated. Growth includes gallery space and priority support.</p></div></div><button className="primary-btn" onClick={onJoinProvider}>Join as a professional</button></>}
  };
...
function UserTerms({onClose}:{onClose:()=>void}) {
  return <section className="terms-sheet" role="dialog" aria-modal="true" aria-labelledby="terms-title">
    <div className="terms-sheet-head"><div><span className="kicker">LUMNEARO USER AGREEMENT</span><h2 id="terms-title">User Terms & Privacy Notice</h2><p>Effective 5 August 2026 · Version {TERMS_VERSION}</p></div><button type="button" onClick={onClose} aria-label="Close terms">×</button></div>
    <div className="terms-content">
      ...
      <h3>1. Who may use LumNearo</h3><p>You must be at least 18 years old, provide accurate account information and keep your password secure. You are responsible for activity under your account.</p>
      <h3>2. Marketplace role</h3><p>LumNearo helps local customers and independent service professionals find and contact each other. Providers are not LumNearo employees. Customers and providers must agree directly on scope, price, materials, timing and payment before work starts.</p>
      ...
      <h3>4. Honest and safe use</h3><p>Do not impersonate others, create fake requests or reviews, misrepresent qualifications, submit unlawful content, demand advance money dishonestly, bypass safety controls or use LumNearo for fraud. Providers must perform only work they are qualified and legally permitted to perform.</p>
      <h3>5. Privacy notice</h3><p>LumNearo processes account details, contact information, approximate or selected location, requests, bookings, messages, profile information and provider verification documents to operate and secure the marketplace. Only information needed for matching and completing a service should be shared. Identity documents and exact coordinates are not intended for public display. LumNearo does not treat acceptance of these terms as consent to unrelated promotional messaging.</p>
      <h3>6. External communication and payments</h3><p>WhatsApp, phone calls and payments outside LumNearo are provided by third parties and carry additional risk. Verify the other user, keep written estimates and receipts, and never share passwords, OTPs, card PINs or banking credentials.</p>
      <h3>7. Reports and enforcement</h3><p>Users should report harassment, spam, fraud, unsafe conduct or privacy misuse to LumNearo administration and preserve relevant evidence. LumNearo may restrict messaging, reject provider verification, suspend or remove accounts and cooperate with lawful authorities when reasonably necessary to protect users or the platform.</p>
      <h3>8. Service disputes and emergencies</h3><p>LumNearo does not guarantee a provider’s work, availability or quoted price. Users remain responsible for deciding whether to hire or accept a job. LumNearo is not an emergency service; contact the appropriate emergency authority when anyone is in immediate danger.</p>
      <h3>9. Changes and law</h3><p>LumNearo may update these terms when the service or legal requirements change and will request acceptance of a new version where appropriate. These terms are governed by applicable laws of India. Nothing here limits rights that cannot lawfully be excluded.</p>
    </div>
...
function CleanDashboard({role,user,onAction,onRequest}:{role:SessionUser["role"];user:SessionUser;onAction:(s:string)=>void;onRequest:()=>void}) {
  ...
  const description = role === "provider"
    ...
    : role === "admin"
      ? "Platform activity will appear here as customers and professionals begin using LumNearo."
...
function ProviderEnquiryCard({item,onUpdated}:{item:ServiceRequest;onUpdated:(id:string,status:string)=>void}) {
  ...
  return <article className="provider-enquiry-card"><div><span className={`status-pill ${assigned?`status-${item.status}`:item.urgency==="Emergency"?"red":"amber"}`}>{assigned?requestStatusLabels[item.status]:item.urgency||"Flexible"}</span><h3>{item.service} request</h3><p>{item.description}</p></div><dl><div><dt>Customer</dt><dd>{item.customerName||"LumNearo customer"}</dd></div><div><dt>Location</dt><dd>{item.address}</dd></div><div><dt>Preferred</dt><dd>{item.preferredDate} · {item.preferredTime}</dd></div><div><dt>Request</dt><dd>{item.requestNumber}</dd></div></dl>{assigned?<div className="assigned-job-actions"><b>{item.status==="accepted"?"The customer selected you":"Work is marked in progress"}</b><p>Keep the customer informed and update the job when work begins or is completed.</p>{error&&<div className="auth-error">{error}</div>}<div>{item.status==="accepted"&&<button className="primary-btn" disabled={busy} onClick={()=>updateJob("start")}>Start work</button>}{item.status==="in_progress"&&<button className="primary-btn" disabled={busy} onClick={()=>updateJob("complete")}>Mark completed</button>}{item.whatsappNumber&&<a className="whatsapp-btn" href={whatsappUrl(item.whatsappNumber,`Hello ${item.customerName||"there"}, I am contacting you about ${item.requestNumber} on LumNearo.`)} target="_blank" rel="noreferrer">WhatsApp customer</a>}</div></div>:<form className="provider-reply-form" onSubmit={reply}><label>Message to customer<textarea value={message} onChange={event=>setMessage(event.target.value)} minLength={3} required placeholder="Introduce yourself and explain how you can help…"/></label><div className="form-row"><label>Estimated price (₹)<input type="number" min="0" max="10000000" value={quoteAmount} onChange={event=>setQuoteAmount(event.target.value)} required placeholder="e.g. 1500"/></label><label>Your availability<input value={availability} onChange={event=>setAvailability(event.target.value)} minLength={3} required placeholder="e.g. Tomorrow morning"/></label></div>{error&&<div className="auth-error">{error}</div>}{sent&&<div className="reply-sent">✓ Reply and estimate saved</div>}<div><button className="primary-btn" disabled={busy}>{busy?"Sending…":"Send reply & estimate"}</button>{item.whatsappNumber&&<a className="whatsapp-btn" href={whatsappUrl(item.whatsappNumber,`Hello ${item.customerName||"there"}, I am responding to your ${item.service} request ${item.requestNumber} on LumNearo.`)} target="_blank" rel="noreferrer">WhatsApp customer</a>}</div></form>}</article>;
}
