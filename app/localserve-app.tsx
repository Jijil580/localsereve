...
function RequestForm({onSuccess,onBeforeAction,provider,language,customerLocation}:{onSuccess:(message:string)=>void;onBeforeAction:(context:string,action:()=>void)=>void;provider?:Provider|null;language:Language;customerLocation?:MapLocation|null}) {
  ...
  const [draft,setDraft]=useState({service:provider?.service||"",description:"",address:customerLocation?.label||"",preferredDate:"",preferredTime:"Morning",urgency:"Flexible",whatsappNumber:"",allowWhatsApp:false,preferredProviderId:provider?.id||"",location:customerLocation||null});
