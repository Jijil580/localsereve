...
function AllServicesCatalogue({onChoose,language}:{onChoose:(service:string)=>void;language:Language}) {
  const t=translations[language];
  const serviceTilePhoto=(service:string)=>{const name=service.toLowerCase();if(/plumb|water-tank|drain|septic|borewell|ro |purifier/.test(name))return "/service-tiles/plumber.png";if(/electric|inverter|ups|solar|generator|dth|antenna|wi-fi|internet|appliance|refrigerator|washing|air-conditioner|television/.test(name))return "/service-tiles/electrician.png";if(/mason|interlock|hollow|tile|marble|granite|floor|plaster|paint|waterproof|roof|false-ceiling|civil|building|architect|structural|surveyor|renovation|demolition|welder|fabrication|aluminium|glass/.test(name))return "/service-tiles/construction.png";if(/carpenter|interior/.test(name))return "/near-lio-carpenter.jpg";if(/clean|pest|laundry/.test(name))return "/service-tiles/cleaning.png";if(/garden|tree|lawn/.test(name))return "/service-tiles/gardening.png";if(/rubber|coconut/.test(name))return "/service-tiles/agriculture.png";if(/security|cctv|automation/.test(name))return "/service-tiles/security.png";if(/mobile|computer|laptop/.test(name))return "/service-tiles/mobile-repair.png";if(/mechanic|car wash|towing|bike/.test(name))return "/service-tiles/mechanic.png";if(/driver|taxi|vehicle|mover|delivery|autorickshaw|traveller|ambulance/.test(name))return "/service-tiles/driver.png";if(/photo|video|drone|event|wedding|decorator/.test(name))return "/service-tiles/photography.png";if(/makeup|beautician|hair|mehendi|barber|tailor/.test(name))return "/service-tiles/beauty.png";if(/catering|cook|chef|chicken|beef|restaurant|retail|lottery/.test(name))return "/service-tiles/food.png";if(/nurse|physio|elder|babysitter|pharmacy|dental|hospital|laboratory/.test(name))return "/service-tiles/healthcare.png";if(/tutor|teacher|fitness|yoga|dance|music|language/.test(name))return "/service-tiles/education.png";if(/web|graphic|digital|account|tax|legal|document|printing|signboard/.test(name))return "/service-tiles/digital.png";return "/near-lio-carpenter.jpg"};
...
function AppModal({type,provider,user,customerLocation,language,authMode,onLocationSaved,onClose,onFindServices,onJoinProvider,onSuccess,onAuthenticated,onProfileSaved}:{type:string;provider:Provider|null;user:SessionUser|null;customerLocation:MapLocation|null;language:Language;authMode:"login"|"register";onLocationSaved:(location:MapLocation)=>void;onClose:()=>void;onFindServices:()=>void;onJoinProvider:()=>void;onSuccess:(m:string)=>void;onAuthenticated:(u:SessionUser)=>void;onProfileSaved:(u:SessionUser)=>void}) {
  ...
  if(type==="request"||type==="booking") return <div className="overlay"><div className="modal auth-modal"><button className="modal-close" onClick={onClose}>×</button>{user?<RequestForm provider={type==="booking"?provider:null} language={language} customerLocation={customerLocation} onSuccess={onSuccess}/>:<AuthForm onAuthenticated={onAuthenticated} language={language}/>}</div></div>;
...
function RequestForm({onSuccess,provider,language,customerLocation}:{onSuccess:(message:string)=>void;provider?:Provider|null;language:Language;customerLocation?:MapLocation|null}) {
...
function CleanMessagesView({onFind,onRequests}:{onFind:()=>void;onRequests:()=>void}) {
  const [requests,setRequests]=useState<ServiceRequest[]>([]);const [loading,setLoading]=useState(true);
  useEffect(()=>{let active=true;fetch("/api/requests",{credentials:"include"}).then(async response=>{const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to load messages");return result.data||[]}).then(data=>{if(active)setRequests(data)}).catch(()=>{if(active)setRequests([])}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[]);
  const replies=requests.flatMap(request=>(request.responses||[]).map(reply=>({request,reply}))).sort((a,b)=>new Date(b.reply.createdAt).getTime()-new Date(a.reply.createdAt).getTime());
  if(loading)return <div className="messages-empty-page"><div className="clean-empty"><span>◌</span><h3>Loading replies</h3><p>Checking for provider quotations and updates.</p></div></div>;
  if(!replies.length)return <div className="messages-empty-page"><div className="clean-empty"><span>✉</span><h3>No replies yet</h3><p>Provider quotations and responses to your service requests will appear here.</p><div className="clean-empty-actions"><button className="secondary-btn" onClick={onRequests}>View requests</button><button className="primary-btn" onClick={onFind}>Find a professional</button></div></div></div>;
  return <div className="messages-empty-page customer-inbox"><div className="inbox-heading"><span className="kicker">YOUR INBOX</span><h1>Provider replies</h1><p>Review quotations and availability sent for your service requests.</p></div><div className="inbox-reply-list">{replies.map(({request,reply})=><article className="inbox-reply-card" key={`${request._id}-${reply.providerId}-${reply.createdAt}`}><div className="inbox-reply-top"><span className="status-pill status-quoted">New reply</span><time>{new Date(reply.createdAt).toLocaleDateString()}</time></div><h3>{reply.providerBusiness||reply.providerName}</h3><p className="inbox-service">Reply to your {request.service} request</p><p className="inbox-message">{reply.message}</p><dl><div><dt>Quote</dt><dd>{reply.quoteAmount?`₹${reply.quoteAmount.toLocaleString("en-IN")}`:"Price on discussion"}</dd></div><div><dt>Availability</dt><dd>{reply.availability||"To be confirmed"}</dd></div></dl><button className="secondary-btn" onClick={onRequests}>View request</button></article>)}</div></div>;
}
