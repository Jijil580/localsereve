    ...
    <div className="app-shell">
      ...
      <header className="topbar">
        ...
        <div className="header-actions">
          ...
          <label className="language-switch"><span className="sr-only">{t.languageLabel}</span><select value={language} onChange={event=>setLanguage(event.target.value as Language)} aria-label={t.languageLabel}><option value="EN">English</option><option value="ML">മലയാളം</option></select></label>
        ...
        {view === "home" && <>
          <section className="hero hero-reference">
            ...
            <div className="hero-collage" aria-label="Verified Nearlio service professionals">
              ...
              <div className="happy-customers"><b>Nearlio</b><span>{t.builtForCommunities}</span></div>
            </div>
          </section>

          <div className="home-service-motion" aria-label="Popular Nearlio services"><div>{[...homeMotionServices,...homeMotionServices].map((service,index)=><span key={`${service}-${index}`}>✦ {serviceLabel(service,language)}</span>)}</div></div>
      ...
      <footer><div className="footer-brand"><div className="brand"><span className="brand-mark">N</span><span className="brand-wordmark"><span>Near<span>lio</span></span><small>by Lumier</small></span></div><p>{t.tagline}</p><small className="powered-by">{t.poweredBy} <b>Lumier Technologies</b></small></div><div><b>{t.customers}</b><button onClick={() => goSearch()}>{t.findServices}</button><button onClick={openRequest}>{t.postRequest}</button><button onClick={() => setModal("safety")}>{t.safety}</button></div><div><b>{t.professionals}</b><button onClick={() => {setRole("provider");currentUser?setView("dashboard"):setModal("auth")}}>{t.joinNearlio}</button><button onClick={() => setModal("pricing")}>{t.plansPricing}</button><button onClick={() => setModal("provider-help")}>{t.providerHelp}</button></div><div><b>{t.company}</b><button onClick={() => setModal("about")}>{t.about}</button><button onClick={() => setModal("contact")}>{t.contact}</button><button onClick={() => setModal("terms")}>{t.privacyTerms}</button></div><div className="footer-bottom">© 2026 Nearlio. {t.tagline} <span>{t.poweredBy} Lumier Technologies · demo@lumiertechnologies.com</span></div></footer>
