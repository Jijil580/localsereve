...
function ProviderEnquiryCard({item,onReplied}:{item:ServiceRequest;onReplied:(id:string)=>void}) {
  ...
  return <article className="provider-enquiry-card"><div><span className={`status-pill ${item.urgency==="Emergency"?"red":"amber"}`}>{item.urgency||"Flexible"}</span><h3>{item.service} request</h3><p>{item.description}</p></div><dl><div><dt>Customer</dt><dd>{item.customerName||"LocalServe customer"}</dd></div><div><dt>Location</dt><dd>{item.address}</dd></div><div><dt>Preferred</dt><dd>{item.preferredDate} · {item.preferredTime}</dd></div><div><dt>Request</dt><dd>{item.requestNumber}</dd></div></dl><form className="provider-reply-form" onSubmit={reply}><label>Reply to customer<textarea value={message} onChange={event=>setMessage(event.target.value)} minLength={3} required placeholder="Introduce yourself, confirm availability, or share an estimate…"/></label>{error&&<div className="auth-error">{error}</div>}{sent&&<div className="reply-sent">✓ Reply sent and saved</div>}<div><button className="primary-btn" disabled={busy}>{busy?"Sending…":"Send reply"}</button>{item.whatsappNumber&&<a className="whatsapp-btn" href={whatsappUrl(item.whatsappNumber,`Hello ${item.customerName||"there"}, I am responding to your ${item.service} request ${item.requestNumber} on LocalServe.`)} target="_blank" rel="noreferrer">WhatsApp customer</a>}</div></form></article>;
}
