...
function CustomerDashboard({user,savedCount,onRequest,onRequests,onMessages}:{user:SessionUser;savedCount:number;onRequest:()=>void;onRequests:()=>void;onMessages:(requestId:string)=>void}){
  ...
  useEffect(()=>{let active=true;setLoading(true);fetch("/api/requests",{credentials:"include"}).then(async response=>{const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to load your account activity");if(active)setRequests(result.data||[])}).catch(problem=>{if(active)setError(problem instanceof Error?problem.message:"Unable to load your account activity")}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[]);
  ...
  const stats=[[String(activeRequests.length),"Active requests"],[String(quotesReceived),"Quotes received"],[String(upcoming.length),"Upcoming bookings"],[String(savedCount),"Saved providers"]];
