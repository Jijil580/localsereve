  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {view === "messages" && currentUser && <CleanMessagesView user={currentUser} onFind={goSearch} onRequests={()=>navigate("requests")} initialRequestId={messageRequestId} />}
        {view === "dashboard" && currentUser && (currentUser.role==="customer"?<CustomerDashboard user={currentUser} savedProviders={providers.filter(provider=>saved.includes(provider.id))} onProvider={setSelected} onRequest={openRequest} onRequests={()=>navigate("requests")} onMessages={(requestId)=>{setMessageRequestId(requestId);navigate("messages")}}/>:<ProviderDashboard role={currentUser.role} user={currentUser} onAction={notify} onRequest={openRequest} onSetup={()=>setModal("profile")} onView={()=>setModal("profile-view")} onMessages={(requestId)=>{setMessageRequestId(requestId);navigate("messages")}} />)}
      </main>
...
function CleanMessagesView({user,onFind,onRequests,initialRequestId=""}:{user:SessionUser;onFind:()=>void;onRequests:()=>void;initialRequestId?:string}) {
  ...
  async function load(silent=false){try{const response=await fetch("/api/messages",{credentials:"include"});const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to load messages");const loaded:Conversation[]=result.data||[];setConversations(loaded);setSelectedId(current=>{const requested=initialRequestId?loaded.find(item=>item.requestId===initialRequestId):null;return requested?.id||(current==="__first__"?(loaded[0]?.id||""):current)})}catch(problem){if(!silent)setError(problem instanceof Error?problem.message:"Unable to load messages")}finally{if(!silent)setLoading(false)}}
  useEffect(()=>{load();const timer=window.setInterval(()=>load(true),8000);return()=>window.clearInterval(timer)},[initialRequestId]);
...
function CustomerDashboard({user,savedProviders,onProvider,onRequest,onRequests,onMessages}:{user:SessionUser;savedProviders:Provider[];onProvider:(provider:Provider)=>void;onRequest:()=>void;onRequests:()=>void;onMessages:(requestId:string)=>void}){
