  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {view === "requests" && currentUser && <RequestsView onPost={openRequest} onMessages={(requestId,providerId)=>{setMessageRequestId(requestId);setMessageProviderId(providerId);navigate("messages")}} />}
        {view === "messages" && currentUser && <CleanMessagesView user={currentUser} onFind={goSearch} onRequests={()=>navigate("requests")} initialRequestId={messageRequestId} initialProviderId={messageProviderId} />}
        {view === "dashboard" && currentUser && (currentUser.role==="customer"?<CustomerDashboard user={currentUser} savedProviders={providers.filter(provider=>saved.includes(provider.id))} onProvider={setSelected} onRequest={openRequest} onRequests={()=>navigate("requests")} onMessages={(requestId,providerId="")=>{setMessageRequestId(requestId);setMessageProviderId(providerId);navigate("messages")}}/>:<ProviderDashboard role={currentUser.role} user={currentUser} onAction={notify} onRequest={openRequest} onSetup={()=>setModal("profile")} onView={()=>setModal("profile-view")} onMessages={(requestId)=>{setMessageRequestId(requestId);setMessageProviderId("");navigate("messages")}} />)}
      </main>
      ...
      <nav className="mobile-nav premium-mobile-nav" aria-label="Mobile navigation">{[["home",t.home,"home"],["requests",t.requests,"requests"],["messages",t.messages,"messages"],["account",t.account,"dashboard"]].map(([icon,label,id]) => <button aria-current={view===id ? "page" : undefined} className={view===id ? "active" : ""} onClick={() => (["requests","messages","dashboard"].includes(id) ? openProtected(id as View) : navigate(id as View))} key={id}><span><MobileNavIcon name={icon}/></span>{label}</button>)}</nav>
...
function CleanMessagesView({user,onFind,onRequests,initialRequestId="",initialProviderId=""}:{user:SessionUser;onFind:()=>void;onRequests:()=>void;initialRequestId?:string;initialProviderId?:string}) {
  const [conversations,setConversations]=useState<Conversation[]>([]);const [selectedId,setSelectedId]=useState("__first__");const [message,setMessage]=useState("");const [completionAmount,setCompletionAmount]=useState("");const [loading,setLoading]=useState(true);const [sending,setSending]=useState(false);const [lifecycleBusy,setLifecycleBusy]=useState(false);const [error,setError]=useState("");
  async function load(silent=false){try{const response=await fetch("/api/messages",{credentials:"include"});const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to load messages");const loaded:Conversation[]=result.data||[];setConversations(loaded);setSelectedId(current=>{const requested=initialRequestId?loaded.find(item=>item.requestId===initialRequestId&&(!initialProviderId||item.providerId===initialProviderId)):null;return requested?.id||(current==="__first__"?(loaded[0]?.id||""):current)})}catch(problem){if(!silent)setError(problem instanceof Error?problem.message:"Unable to load messages")}finally{if(!silent)setLoading(false)}}
