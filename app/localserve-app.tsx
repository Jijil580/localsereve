  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {view === "dashboard" && currentUser && <ProviderDashboard role={role} user={currentUser} onAction={notify} onSetup={()=>setModal("profile")} onView={()=>setModal("profile-view")} />}
      </main>
...
function ProviderDashboard({role,user,onAction,onSetup,onView}:{role:SessionUser["role"];user:SessionUser;onAction:(s:string)=>void;onSetup:()=>void;onView:()=>void}) {
...
function LocationPicker({value,onChange}:{value:MapLocation|null;onChange:(location:MapLocation)=>void}) {
  ...
  return <div className="location-picker"><form className="place-search" onSubmit={searchPlace}><input value={placeQuery} onChange={event=>setPlaceQuery(event.target.value)} placeholder="Search area, town, landmark or postcode" aria-label="Search location"/><button disabled={searching}>{searching?"Searching…":"Search"}</button></form>{placeResults.length>0&&<div className="place-results">{placeResults.map((result,index)=><button type="button" key={`${result.latitude}-${result.longitude}-${index}`} onClick={()=>{onChange(result);setPlaceQuery(result.label||"");setPlaceResults([])}}><b>⌖</b><span>{result.label}</span></button>)}</div>}<button type="button" className="gps-button" onClick={detect} disabled={locating}>◎ {locating?"Detecting your location…":"Use my current mobile location"}</button><div ref={containerRef} className="map-canvas" aria-label="Choose location on map"/>{value&&<div className="coordinate-chip">✓ Location selected · {value.label||`${value.latitude.toFixed(5)}, ${value.longitude.toFixed(5)}`}</div>}{error&&<div className="location-error">{error}</div>}</div>;
}
