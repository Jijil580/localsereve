...
function CleanMessagesView({user,onFind,onRequests,initialRequestId="",initialProviderId=""}:{user:SessionUser;onFind:()=>void;onRequests:()=>void;initialRequestId?:string;initialProviderId?:string}) {
  const [conversations,setConversations]=useState<Conversation[]>([]);const [selectedId,setSelectedId]=useState("");const [message,setMessage]=useState("");const [completionAmount,setCompletionAmount]=useState("");const [loading,setLoading]=useState(true);const [sending,setSending]=useState(false);const [lifecycleBusy,setLifecycleBusy]=useState(false);const [error,setError]=useState("");
  async function load(silent=false){try{const response=await fetch("/api/messages",{credentials:"include"});const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to load messages");const loaded:Conversation[]=result.data||[];setConversations(loaded);setSelectedId(current=>{const requested=initialRequestId?loaded.find(item=>item.requestId===initialRequestId&&(!initialProviderId||item.providerId===initialProviderId)):null;if(requested)return requested.id;return loaded.some(item=>item.id===current)?current:""})}catch(problem){if(!silent)setError(problem instanceof Error?problem.message:"Unable to load messages")}finally{if(!silent)setLoading(false)}}
  useEffect(()=>{load();const timer=window.setInterval(()=>load(true),8000);return()=>window.clearInterval(timer)},[initialRequestId,initialProviderId]);
  const selected=conversations.find(item=>item.id===selectedId);
  const ownCompletionConfirmed=Boolean(user.role==="customer"?selected?.customerCompletionConfirmedAt:selected?.providerCompletionConfirmedAt);
  const otherCompletionConfirmed=Boolean(user.role==="customer"?selected?.providerCompletionConfirmedAt:selected?.customerCompletionConfirmedAt);
  ...
  useEffect(()=>{if(!selected)return;fetch("/api/messages",{method:"PATCH",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({requestId:selected.requestId,providerId:selected.providerId})}).then(response=>{if(response.ok)window.dispatchEvent(new Event("nearleo:messages-read"))}).catch(()=>{});setConversations(current=>current.map(item=>item.id===selected.id?{...item,unread:0}:item))},[selected?.id]);
  ...
  async function updateLifecycle(action:"accept"|"confirm"|"start"|"complete"|"cancel"){if(!selected)return;if(action==="cancel"&&!window.confirm("Cancel this job? Live travel will stop immediately for both people."))return;setLifecycleBusy(true);setError("");try{const finalAmount=action==="complete"?Number(completionAmount):undefined;if(action==="complete"&&(completionAmount===""||!Number.isFinite(finalAmount)||Number(finalAmount)<0||Number(finalAmount)>10000000)){setError("Enter a valid final amount to complete this job.");return}const response=await fetch(`/api/requests/${selected.requestId}`,{method:"PATCH",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({action,providerId:selected.providerId,finalAmount})});const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to update the work status");const assignedProviderId=String(result.data.assignedProviderId||selected.assignedProviderId||"");setConversations(current=>current.map(item=>item.requestId===selected.requestId?{...item,status:result.data.status,assignedProviderId,isSelected:item.providerId===assignedProviderId,confirmedAt:result.data.confirmedAt??item.confirmedAt,arrivedAt:result.data.arrivedAt??item.arrivedAt,startedAt:result.data.startedAt??item.startedAt,completedAt:result.data.completedAt??item.completedAt,cancelledAt:result.data.cancelledAt??item.cancelledAt,cancelledByRole:result.data.cancelledByRole??item.cancelledByRole,customerCompletionConfirmedAt:result.data.customerCompletionConfirmedAt??item.customerCompletionConfirmedAt,providerCompletionConfirmedAt:result.data.providerCompletionConfirmedAt??item.providerCompletionConfirmedAt,paymentGivenAt:result.data.paymentGivenAt??item.paymentGivenAt,paymentReceivedAt:result.data.paymentReceivedAt??item.paymentReceivedAt,finalAmount:Number(result.data.finalAmount??item.finalAmount)}:item))}catch(problem){setError(problem instanceof Error?problem.message:"Unable to update the work status")}finally{setLifecycleBusy(false)}}
  ...
  return <section className="conversation-app" aria-label="Nearleo messages"><aside className={`conversation-list ${selected?"has-selection":""}`}><div className="conversation-list-head"><span className="kicker">NEARLEO INBOX</span><h1>Messages</h1><p>{conversations.reduce((sum,item)=>sum+item.unread,0)} unread updates</p></div>{conversations.map(item=>{const title=user.role==="provider"?item.customerName:item.providerName;return <button type="button" className={`${item.id===selected?.id?"active":""} ${item.unread?"unread":""} ${item.isSelected?"selected-provider-chat":""}`} key={item.id} onClick={()=>setSelectedId(item.id)}><MessageAvatar name={title} photoUrl={user.role==="provider"?"":item.providerPhotoUrl}/><span><b>{title}{item.isSelected&&<i>Selected</i>}</b><small>{item.service} · {item.requestNumber}</small><em>{item.lastMessage||"Open conversation"}</em></span><time>{new Date(item.updatedAt).toLocaleDateString(undefined,{day:"2-digit",month:"short"})}</time>{item.unread>0&&<i>{item.unread}</i>}</button>})}</aside>{selected&&<div className="conversation-thread"><header><button type="button" className="conversation-back" onClick={()=>setSelectedId("")} aria-label="Back to conversations">←</button><MessageAvatar name={user.role==="provider"?selected.customerName:selected.providerName} photoUrl={user.role==="provider"?"":selected.providerPhotoUrl}/><div><b>{user.role==="provider"?selected.customerName:selected.providerName}</b><small>{selected.service} · {selected.requestNumber}</small></div><span className={`status-pill status-${selected.status}`}>{requestStatusLabels[selected.status]||selected.status.replace("_"," ")}</span></header>{selected.status==="completed"?<div className="conversation-quote completed-payment"><span><small>Amount paid</small><b>₹{Number(selected.finalAmount||0).toLocaleString("en-IN")}</b></span><span><small>Work status</small><b>Completed{selected.completedAt?` · ${new Date(selected.completedAt).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"})}`:""}</b></span></div>:Boolean(selected.quoteAmount||selected.availability)&&<div className={`conversation-quote ${user.role==="customer"?"customer-estimate-highlight":""}`}><span><small>{user.role==="customer"?"PROVIDER ESTIMATE":"Estimate"}</small><b>{selected.quoteAmount?`₹${selected.quoteAmount.toLocaleString("en-IN")}`:"Discuss price"}</b></span><span><small>Availability</small><b>{selected.availability||"To be confirmed"}</b></span>{user.role==="customer"&&<em>Review this estimate, then confirm this provider or compare another reply.</em>}</div>}{user.role==="customer"&&["open","quoted"].includes(selected.status)?<div className="conversation-lifecycle"><span><b>Ready to hire this professional?</b><small>Select only after discussing the work, price and availability.</small></span><button type="button" disabled={lifecycleBusy} onClick={()=>updateLifecycle("accept")}>{lifecycleBusy?"Selecting…":"Mark as selected"}</button></div>:selected.status==="accepted"&&selected.isSelected?<div className="conversation-lifecycle selected"><span><b>{user.role==="provider"?"You are selected for this request":"Provider selected"}</b><small>{user.role==="provider"?"Waiting for the customer to confirm the job.":"Confirm only after the provider agrees to the work, price and schedule."}</small></span>{user.role==="customer"&&<button type="button" disabled={lifecycleBusy} onClick={()=>updateLifecycle("confirm")}>{lifecycleBusy?"Confirming…":"Confirm job"}</button>}</div>:selected.status==="accepted"&&!selected.isSelected&&user.role==="customer"?<div className="conversation-lifecycle muted"><span><b>Another provider is selected</b><small>This conversation remains available for your records.</small></span></div>:selected.status==="confirmed"&&selected.isSelected?<div className="conversation-lifecycle confirmed"><span><b>Job confirmed</b><small>The customer and provider can mark the work as started when it begins.</small></span><button type="button" disabled={lifecycleBusy} onClick={()=>updateLifecycle("start")}>{lifecycleBusy?"Updating…":"Mark work in progress"}</button></div>:selected.status==="in_progress"&&selected.isSelected?<div className="conversation-lifecycle in-progress"><span><b>Work in progress</b><small>{ownCompletionConfirmed?`${user.role==="customer"?"Payment given":"Payment received"} confirmed. Waiting for the ${user.role==="customer"?"provider":"customer"} to confirm the same amount.`:otherCompletionConfirmed?`The ${user.role==="customer"?"provider":"customer"} confirmed ₹${Number(selected.finalAmount||0).toLocaleString("en-IN")}. Enter the same amount to finish the job.`:"When the work is finished, both customer and provider must confirm the final amount."}</small></span>{ownCompletionConfirmed?<span className="completion-waiting">✓ {user.role==="customer"?"Payment given":"Payment received"} · Waiting for confirmation</span>:<><label className="conversation-completion-amount"><span>{user.role==="customer"?"Final amount paid (₹)":"Final amount received (₹)"}</span><input type="number" min="0" max="10000000" inputMode="numeric" value={completionAmount} onChange={event=>setCompletionAmount(event.target.value)} placeholder="Enter final amount"/></label><button type="button" disabled={lifecycleBusy||completionAmount===""||!Number.isFinite(Number(completionAmount))||Number(completionAmount)<0} onClick={()=>updateLifecycle("complete")}>{lifecycleBusy?"Saving…":user.role==="customer"?"Work finished · Payment given":"Work finished · Payment received"}</button></>}</div>:selected.status==="completed"&&selected.isSelected?<div className="conversation-lifecycle completed"><span><b>Job completed</b><small>The final amount is saved in both account histories.</small></span></div>:selected.status==="cancelled"&&selected.isSelected?<div className="conversation-lifecycle cancelled"><span><b>Job cancelled</b><small>Live travel and job actions are now closed for both customer and provider.</small></span></div>:null}{selected.isSelected&&["accepted","confirmed","in_progress"].includes(selected.status)&&<div className="conversation-cancel-job"><span>Need to stop this job?</span><button type="button" disabled={lifecycleBusy} onClick={()=>updateLifecycle("cancel")}>{lifecycleBusy?"Cancelling…":"Cancel job"}</button></div>}<JobTrackingPanel conversation={selected} user={user}/><div className="conversation-messages">{selected.messages.map(item=>{const mine=item.senderUserId===user.id||item.senderRole===user.role;return <div className={`chat-bubble ${mine?"mine":"theirs"}`} key={item.id}><p>{item.text}</p><span>{new Date(item.createdAt).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}{mine&&<i>✓✓</i>}</span></div>})}</div>{error&&<div className="conversation-error">{error}</div>}<form className="conversation-composer" onSubmit={send}><label><span className="sr-only">Type a message</span><textarea value={message} onChange={event=>setMessage(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"&&!event.shiftKey){event.preventDefault();event.currentTarget.form?.requestSubmit()}}} rows={1} maxLength={1500} placeholder="Type a message…"/></label><button disabled={sending||!message.trim()} aria-label="Send message">{sending?"…":"➤"}</button></form></div>}</section>;
}
...
function ProviderDashboard({role,user,onAction,onRequest,onSetup,onView,onMessages}:{role:SessionUser["role"];user:SessionUser;onAction:(s:string)=>void;onRequest:()=>void;onSetup:()=>void;onView:()=>void;onMessages:(requestId:string)=>void}) {
  ...
  return <div className="dash-page">
    <div className="page-heading"><div><span className="kicker">PROVIDER DASHBOARD</span><h1>Welcome, {firstName}!</h1><p>New requests appear only until you send your first reply. Replied customers stay available under Active customers.</p></div><div className="page-heading-actions"><button className="ghost-btn" onClick={onView}>View profile</button><button className="primary-btn" onClick={onSetup}>Set up profile</button></div></div>
