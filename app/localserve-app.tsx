...
function JobTrackingPanel({conversation,user}:{conversation:Conversation;user:SessionUser}){
  ...
  async function publishPosition(position:GeolocationPosition){const response=await fetch(`/api/requests/${conversation.requestId}/tracking`,{method:"PUT",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({latitude:position.coords.latitude,longitude:position.coords.longitude,accuracy:position.coords.accuracy})});const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to share your location");setTracking(current=>({...current,status:conversation.status,address:current?.address||"",targetLocation:current?.targetLocation||null,sharing:true,providerLocation:result.data.providerLocation}))}
  function startSharing(){if(!navigator.geolocation){setError("Location is not supported on this device.");return}setBusy(true);setError("");navigator.geolocation.getCurrentPosition(position=>{void publishPosition(position).then(()=>{lastSentAt.current=Date.now();const id=navigator.geolocation.watchPosition(next=>{if(Date.now()-lastSentAt.current<8000)return;lastSentAt.current=Date.now();void publishPosition(next).catch(()=>setError("Live location update paused. Check GPS and internet."))},()=>setError("Live location update paused. Keep location permission enabled."),{enableHighAccuracy:true,maximumAge:5000,timeout:15000});watchId.current=id;setSharingDevice(true)}).catch(problem=>setError(problem instanceof Error?problem.message:"Unable to share your location")).finally(()=>setBusy(false))},problem=>{setBusy(false);setError(problem.code===1?"Allow location permission to share your journey with the customer.":"Your current location could not be detected.")},{enableHighAccuracy:true,maximumAge:0,timeout:15000})}
