...
export default function NearleoApp() {
  ...
  return (
    <div className="app-shell">
      ...
      {selected && !modal && <ProviderDrawer provider={selected} user={currentUser} saved={saved.includes(selected.id)} onClose={() => setSelected(null)} onBook={() => setModal("booking")} onSignIn={openContactSignIn} onSave={() => setSaved(s => s.includes(selected.id) ? s.filter(x=>x!==selected.id) : [...s,selected.id])} />}
...
function ProviderDrawer({provider:p,user,saved,onClose,onBook,onSave,onSignIn}:{provider:Provider;user:SessionUser|null;saved:boolean;onClose:()=>void;onBook:()=>void;onSave:()=>void;onSignIn:()=>void}) {
