...
export default function NearleoApp() {
  ...
  return (
    <div className="app-shell">
      ...
      {sideMenuOpen&&<><button type="button" className="side-menu-backdrop" aria-label="Close navigation menu" onClick={()=>setSideMenuOpen(false)}/><aside id="nearleo-side-menu" className="side-menu" aria-label="Nearleo quick navigation"><div className="side-menu-head"><div className="side-menu-brand"><span className="brand-mark">N</span><span><b>Nearleo</b></span></div><button type="button" aria-label="Close menu" onClick={()=>setSideMenuOpen(false)}>×</button></div><nav aria-label="Quick links"><span className="side-menu-section-label">Menu</span><button type="button" onClick={()=>navigate("home")}><span>⌂</span><div><b>{t.home}</b><small>Return to the Nearleo home page</small></div><i>›</i></button><button type="button" onClick={()=>goSearch("All services")}><span>▦</span><div><b>{t.allServices}</b><small>Browse every local service</small></div><i>›</i></button><button type="button" onClick={()=>{setSideMenuOpen(false);currentUser?(setRole(currentUser.role),navigate("dashboard")):(setAuthMode("login"),setModal("auth"))}}><span>○</span><div><b>{currentUser?t.profileDashboard:t.signIn}</b><small>{currentUser?t.manageAccount:"Access your account or create one"}</small></div><i>›</i></button><button type="button" onClick={()=>{setSideMenuOpen(false);openProtected("requests")}}><span>＋</span><div><b>{t.myRequests}</b><small>Track requests, replies and jobs</small></div><i>›</i></button><button type="button" onClick={()=>{setSideMenuOpen(false);setModal("about")}}><span>i</span><div><b>{t.about}</b><small>How Nearleo helps local customers</small></div><i>›</i></button><button type="button" onClick={()=>{setSideMenuOpen(false);setModal("provider-help")}}><span>?</span><div><b>Help &amp; support</b><small>Simple help and safety guidance</small></div><i>›</i></button><button type="button" onClick={()=>{setSideMenuOpen(false);setModal("contact")}}><span>@</span><div><b>{t.contact}</b><small>support@nealeo.com</small></div><i>›</i></button></nav><button type="button" className="side-menu-listing" onClick={()=>{setSideMenuOpen(false);setRole("provider");currentUser?navigate("dashboard"):setModal("auth")}}>List your service <span>→</span></button></aside></>}
