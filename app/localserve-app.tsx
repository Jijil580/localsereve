...
function ProviderEnquiryCard({item,onUpdated,onMessages}:{item:ServiceRequest;onUpdated:(id:string,changes:Partial<ServiceRequest>)=>void;onMessages:()=>void}) {
  ...
  async function reply(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);setError("");try{const sentMessage=message;const sentQuote=Number(quoteAmount);const sentAvailability=availability;const response=await fetch(`/api/provider/requests/${item._id}/respond`,{method:"POST",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({message:sentMessage,quoteAmount:sentQuote,availability:sentAvailability})});const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to send reply");const savedReply:RequestReply={providerId:"current-provider",providerName:"",providerBusiness:"",message:sentMessage,quoteAmount:sentQuote,availability:sentAvailability,createdAt:new Date().toISOString()};setMessage("");setQuoteAmount("");setAvailability("");setSent(true);onUpdated(item._id,{status:"quoted",quoteCount:item.quoteCount+1,responses:[...(item.responses||[]),savedReply]})}catch(problem){setError(problem instanceof Error?problem.message:"Unable to send reply")}finally{setBusy(false)}}
