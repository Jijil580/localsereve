...
export default function NearleoApp() {
  ...
  const [postAuthAction,setPostAuthAction]=useState<View|"request"|"call"|null>(null);
  ...
  function requestVoiceCall(provider:Provider){setSelected(null);setRequestedCallProvider({id:provider.id,business:provider.business,service:provider.service});if(!currentUser){setAuthMode("login");setPostAuthAction("call");setModal("auth");return;}if(currentUser.role!=="customer"){setRequestedCallProvider(null);notify("Sign in with a customer account to call a professional.");}}
  ...
  return (
    <div className="app-shell">
      ...
      {selected && !modal && <ProviderDrawer provider={selected} user={currentUser} saved={saved.includes(selected.id)} onClose={() => setSelected(null)} onBook={() => setModal("booking")} onCall={()=>requestVoiceCall(selected)} onSignIn={() => setModal("auth")} onSave={() => setSaved(s => s.includes(selected.id) ? s.filter(x=>x!==selected.id) : [...s,selected.id])} />}
      {modal && <AppModal type={modal} provider={selected} user={currentUser} customerLocation={customerLocation} language={language} authMode={authMode} onLocationSaved={saveCustomerLocation} onClose={() => {setModal(null);setPostAuthAction(null);if(postAuthAction==="call")setRequestedCallProvider(null)}} onFindServices={() => {setModal(null);goSearch()}} onJoinProvider={() => {setRole("provider");if(currentUser){setModal(null);setView("dashboard")}else setModal("auth")}} onAuthenticated={(user) => {setCurrentUser(user);setRole(user.role);notify(`Welcome, ${user.fullName.split(" ")[0]}!`);if(postAuthAction==="request"){setModal("request")}else if(postAuthAction==="call"){setModal(null)}else if(modal==="booking"){setModal("booking")}else{setModal(null);setView(postAuthAction||"dashboard")}setPostAuthAction(null)}} onProfileSaved={(user) => {setCurrentUser(user);setRole("provider");setModal(null);refreshProviders();notify("Profile saved and published on Nearleo.");setView("dashboard")}} onSuccess={(msg) => {setModal(null); notify(msg); if(modal!=="auth")setView("requests")}} />}
...
function WhatsAppAccess({provider,user,onSignIn}:{provider:Provider;user:SessionUser|null;onSignIn:()=>void}) {
  ...
  if(!user)return <div className="whatsapp-access"><b>Private WhatsApp contact</b><span>Sign in as a customer to request permission from this provider.</span><button type="button" onClick={onSignIn}>Sign in to request WhatsApp contact</button></div>;
  ...
  return <div className={`whatsapp-access status-${status}`}><b>Private WhatsApp contact</b>{status==="loading"?<span>Checking permission…</span>:status==="pending"?<span>Request Pending — the provider will approve or decline.</span>:status==="declined"?<span>WhatsApp contact request declined.</span>:status==="approved"?<><span>WhatsApp Messaging Active</span><a className="whatsapp-btn" href={url} target="_blank" rel="noreferrer">WhatsApp Message</a></>:<><span>The provider’s number stays private until they approve your request.</span><button type="button" onClick={requestAccess}>Request WhatsApp Contact</button></>}{error&&<small>{error}</small>}</div>;
}
