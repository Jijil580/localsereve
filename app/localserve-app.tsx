...
function OpeningIntro({onFinish}:{onFinish:()=>void}) {
  ...
  return <section className="opening-intro" role="status" aria-label={`Nearlio welcome, part ${chapter+1} of 3`} onClick={event=>{if(!(event.target as HTMLElement).closest("button"))nextChapter()}}>
    ...
    <button type="button" className="intro-next" onClick={nextChapter}>{chapter===2?"Enter Nearlio":"Next →"}</button>
    ...
    <div className="app-shell">
      ...
      <header className="topbar">
        ...
        <nav className="desktop-nav" aria-label="Main navigation">
          <button aria-current={view === "home" ? "page" : undefined} className={view === "home" ? "active" : ""} onClick={() => setView("home")}>Home</button>
          <button aria-current={view === "search" ? "page" : undefined} className={view === "search" ? "active" : ""} onClick={() => goSearch()}>Find services</button>
          <button aria-current={view === "requests" ? "page" : undefined} className={view === "requests" ? "active" : ""} onClick={() => openProtected("requests")}>My requests</button>
          <button aria-current={view === "messages" ? "page" : undefined} className={view === "messages" ? "active" : ""} onClick={() => openProtected("messages")}>Messages</button>
        </nav>
      ...
      <main>
        {view === "home" && <>
          <section className="hero hero-reference">
            <div className="hero-copy">
              ...
              <form className="hero-search hero-search-expanded" onSubmit={e => {e.preventDefault(); goSearch()}}>
                <label className="search-field"><span>⌕</span><span><small>SERVICE</small><ServiceAutocomplete value={query} onChange={setQuery} onSelect={goSearch} placeholder="Plumber, electrician, carpenter…" ariaLabel="Service search" /></span></label>
              ...
              <div className="hero-proof-row"><div><span>✓</span><b>Admin-reviewed<br/>profiles</b></div><div><span>₹</span><b>Provider-set<br/>starting prices</b></div><div><span>⌖</span><b>Location-based<br/>discovery</b></div><div><span>↯</span><b>Saved requests<br/>and replies</b></div></div>
            </div>
            <div className="hero-collage" aria-label="Verified Nearlio service professionals">
              <article className="collage-card collage-electrician"><img src="https://images.unsplash.com/photo-1621905252507-b35492cc74b4?auto=format&fit=crop&w=700&q=85" alt="Electrician at work"/><span><b>Electrician</b><small>Local expertise</small></span></article>
              <article className="collage-card collage-main"><img src="/near-lio-carpenter.jpg" alt="Carpenter planing a wooden cabinet"/><span><b>Carpenter</b><small>Skilled service</small></span></article>
              <article className="collage-card collage-tile"><img src="/near-lio-tile-worker.jpg" alt="Tile worker installing wall tiles"/><span><b>Tile worker</b><small>Nearby professionals</small></span></article>
              <article className="collage-card collage-plaster"><img src="/near-lio-plastering-worker.jpg" alt="Plastering worker smoothing an interior wall"/><span><b>Plastering</b><small>Approved profiles</small></span></article>
              <article className="collage-card collage-photographer"><img src="/near-lio-photographer.jpg" alt="Professional cameraman filming an event"/><span><b>Photographer</b><small>Direct enquiries</small></span></article>
              <div className="happy-customers"><b>Nearlio</b><span>Built for local communities</span></div>
            </div>
          </section>
          ...
          <section className="section provider-section">
            <div className="section-head"><div><span className="kicker">APPROVED PROFILES</span><h2>{t.nearby}</h2><p>Real professionals published after Nearlio admin review.</p></div><button onClick={() => goSearch()}>See all professionals →</button></div>
      ...
      <footer><div className="footer-brand"><div className="brand"><span className="brand-mark">N</span><span className="brand-wordmark"><span>Near<span>lio</span></span><small>by Lumier</small></span></div><p>Where Local Experts Meet Local Customers.</p><small className="powered-by">Powered by <b>Lumier Technologies</b></small></div><div><b>Customers</b><button onClick={() => goSearch()}>Find services</button><button onClick={openRequest}>Post a request</button><button onClick={() => setModal("safety")}>Safety</button></div><div><b>Professionals</b><button onClick={() => {setRole("provider");currentUser?setView("dashboard"):setModal("auth")}}>Join Nearlio</button><button onClick={() => setModal("pricing")}>Plans & pricing</button><button onClick={() => setModal("provider-help")}>Provider help</button></div><div><b>Company</b><button onClick={() => setModal("about")}>About</button><button onClick={() => setModal("contact")}>Contact</button><button onClick={() => setModal("terms")}>Privacy & terms</button></div><div className="footer-bottom">© 2026 Nearlio. Where Local Experts Meet Local Customers. <span>Powered by Lumier Technologies · demo@lumiertechnologies.com</span></div></footer>

      <nav className="mobile-nav" aria-label="Mobile navigation">{[["⌂","Home","home"],["⌕","Explore","search"],["＋","Requests","requests"],["✉","Inbox","messages"],["◉","Account","dashboard"]].map(([icon,label,id]) => <button aria-current={view===id ? "page" : undefined} className={view===id ? "active" : ""} onClick={() => (["requests","messages","dashboard"].includes(id) ? openProtected(id as View) : setView(id as View))} key={id}><span>{icon}</span>{label}</button>)}</nav>
      ...
      {modal && <AppModal type={modal} provider={selected} user={currentUser} customerLocation={customerLocation} onLocationSaved={saveCustomerLocation} onClose={() => {setModal(null);setPostAuthAction(null)}} onFindServices={() => {setModal(null);goSearch()}} onJoinProvider={() => {setRole("provider");if(currentUser){setModal(null);setView("dashboard")}else setModal("auth")}} onAuthenticated={(user) => {setCurrentUser(user);setRole(user.role);notify(`Welcome, ${user.fullName.split(" ")[0]}!`);if(postAuthAction==="request"){setModal("request")}else if(modal==="booking"){setModal("booking")}else{setModal(null);setView(postAuthAction||"dashboard")}setPostAuthAction(null)}} onProfileSaved={(user) => {setCurrentUser(user);setRole("provider");setModal(null);refreshProviders();notify("Profile submitted for Nearlio verification.");setView("dashboard")}} onSuccess={(msg) => {setModal(null); notify(msg); if(modal!=="auth")setView("requests")}} />}
...
function AppModal({type,provider,user,customerLocation,onLocationSaved,onClose,onFindServices,onJoinProvider,onSuccess,onAuthenticated,onProfileSaved}:{type:string;provider:Provider|null;user:SessionUser|null;customerLocation:MapLocation|null;onLocationSaved:(location:MapLocation)=>void;onClose:()=>void;onFindServices:()=>void;onJoinProvider:()=>void;onSuccess:(m:string)=>void;onAuthenticated:(u:SessionUser)=>void;onProfileSaved:(u:SessionUser)=>void}) {
  ...
  if(type==="request"||type==="booking") return <div className="overlay"><div className="modal auth-modal"><button className="modal-close" onClick={onClose}>×</button>{user?<RequestForm provider={type==="booking"?provider:null} onSuccess={onSuccess}/>:<AuthForm onAuthenticated={onAuthenticated}/>}</div></div>;
  return <div className="overlay"><div className="modal auth-modal"><button className="modal-close" onClick={onClose}>×</button><AuthForm onAuthenticated={onAuthenticated}/></div></div>;
}
...
function ProviderEnquiryCard({item,onUpdated}:{item:ServiceRequest;onUpdated:(id:string,status:string)=>void}) {
  ...
  return <article className="provider-enquiry-card"><div><span className={`status-pill ${assigned?`status-${item.status}`:item.urgency==="Emergency"?"red":"amber"}`}>{assigned?requestStatusLabels[item.status]:item.urgency||"Flexible"}</span><h3>{item.service} request</h3><p>{item.description}</p></div><dl><div><dt>Customer</dt><dd>{item.customerName||"Nearlio customer"}</dd></div><div><dt>Location</dt><dd>{item.address}</dd></div><div><dt>Preferred</dt><dd>{item.preferredDate} · {item.preferredTime}</dd></div><div><dt>Request</dt><dd>{item.requestNumber}</dd></div></dl>{assigned?<div className="assigned-job-actions"><b>{item.status==="accepted"?"The customer selected you":"Work is marked in progress"}</b><p>Keep the customer informed and update the job when work begins or is completed.</p>{error&&<div className="auth-error">{error}</div>}<div>{item.status==="accepted"&&<button className="primary-btn" disabled={busy} onClick={()=>updateJob("start")}>Start work</button>}{item.status==="in_progress"&&<button className="primary-btn" disabled={busy} onClick={()=>updateJob("complete")}>Mark completed</button>}{item.whatsappNumber&&<a className="whatsapp-btn" href={whatsappUrl(item.whatsappNumber,`Hello ${item.customerName||"there"}, I am contacting you about ${item.requestNumber} on Nearlio.`)} target="_blank" rel="noreferrer">WhatsApp customer</a>}</div></div>:<form className="provider-reply-form" onSubmit={reply}><label>Message to customer<textarea value={message} onChange={event=>setMessage(event.target.value)} minLength={3} required placeholder="Introduce yourself and explain how you can help…"/></label><div className="form-row"><label>Estimated price (₹)<input type="number" min="0" max="10000000" value={quoteAmount} onChange={event=>setQuoteAmount(event.target.value)} required placeholder="e.g. 1500"/></label><label>Your availability<input value={availability} onChange={event=>setAvailability(event.target.value)} minLength={3} required placeholder="e.g. Tomorrow morning"/></label></div>{error&&<div className="auth-error">{error}</div>}{sent&&<div className="reply-sent">✓ Reply and estimate saved</div>}<div><button className="primary-btn" disabled={busy}>{busy?"Sending…":"Send reply & estimate"}</button>{item.whatsappNumber&&<a className="whatsapp-btn" href={whatsappUrl(item.whatsappNumber,`Hello ${item.customerName||"there"}, I am responding to your ${item.service} request ${item.requestNumber} on Nearlio.`)} target="_blank" rel="noreferrer">WhatsApp customer</a>}</div></form>}</article>;
}
