...
export async function GET() {
  ...
  const user = await db.collection("users").findOne({ _id: new ObjectId(session.id) }, { projection: { location: 1, locationLabel: 1 } });
  const point = user?.location?.coordinates;
  return Response.json({ location: Array.isArray(point) ? { longitude: Number(point[0]), latitude: Number(point[1]), label: String(user?.locationLabel ?? "Saved location") } : null });
}
...
export async function PUT(request: Request) {
  ...
  const db = await getMongoDb();
  await db.collection("users").updateOne({ _id: new ObjectId(session.id) }, { $set: { location: { type: "Point", coordinates: [point.longitude, point.latitude] }, locationLabel: label, locationUpdatedAt: new Date() } });
  return Response.json({ ok: true, location: { ...point, label } });
}
