...
export async function GET(){
  ...
  try{
    const db=await getMongoDb();const userId=new ObjectId(session.id);let profileId:ObjectId|null=null;
    if(session.role==="provider"){
      const profile=await db.collection("providers").findOne({userId,status:{$ne:"disabled"}},{projection:{_id:1}});if(profile?._id instanceof ObjectId)profileId=profile._id;
    }
    const participantQueries:Record<string,unknown>[]=[{customerId:userId}];
    if(profileId)participantQueries.push({"responses.providerId":profileId},{"messages.providerId":profileId},{assignedProviderId:profileId});
    const rows=await db.collection("serviceRequests").find(participantQueries.length===1?participantQueries[0]:{$or:participantQueries}).sort({updatedAt:-1}).limit(100).toArray();
    ...
    for(const row of rows){
      const viewerRole=String(row.customerId||"")===session.id?"customer":"provider";
      ...
      for(const providerId of providerIds){
        if(!providerId||(viewerRole==="provider"&&providerId!==String(profileId||"")))continue;
...
export async function POST(request:Request){
  ...
  try{
    ...
    const isCustomer=String(record.customerId)===session.id;let allowedProviderId:ObjectId|null=null;let viewerRole:"customer"|"provider"|null=null;
    if(isCustomer){const hasReply=(Array.isArray(record.responses)?record.responses:[]).some((reply:{providerId?:ObjectId})=>String(reply.providerId||"")===providerId);if(hasReply||String(record.assignedProviderId||"")===providerId){allowedProviderId=new ObjectId(providerId);viewerRole="customer"}}
      ...
      if(profile&&(hasReply||isAssigned)){allowedProviderId=profile._id as ObjectId;viewerRole="provider"}
    }
...
export async function PATCH(request:Request){
  ...
  const db=await getMongoDb();const record=await db.collection("serviceRequests").findOne({_id:new ObjectId(requestId)},{projection:{customerId:1,responses:1,messages:1,assignedProviderId:1}});if(!record)return Response.json({error:"Request not found"},{status:404});
  const isCustomer=String(record.customerId||"")===session.id;let viewerRole:"customer"|"provider"|null=isCustomer?"customer":null;
  if(!viewerRole&&session.role==="provider"){
    const profile=await db.collection("providers").findOne({userId:new ObjectId(session.id),_id:new ObjectId(providerId),status:{$ne:"disabled"}},{projection:{_id:1}});
    const participates=(Array.isArray(record.responses)?record.responses:[]).some((reply:{providerId?:ObjectId})=>String(reply.providerId||"")===providerId)||(Array.isArray(record.messages)?record.messages:[]).some((message:RawMessage)=>String(message.providerId||"")===providerId)||String(record.assignedProviderId||"")===providerId;
    if(profile&&participates)viewerRole="provider";
  }
  if(!viewerRole)return Response.json({error:"You cannot access this conversation"},{status:403});
  const field=viewerRole==="provider"?"messages.$[message].readByProvider":"messages.$[message].readByCustomer";
