...
export async function POST(request: Request) {
  ...
  try {
    ...
    const now = new Date();
    const result = await users.insertOne({ fullName, email, phone, passwordHash, role, status: "active", emailVerified: false, phoneVerified: false, termsVersion: TERMS_VERSION, termsAcceptedAt: now, createdAt: now, updatedAt: now });
