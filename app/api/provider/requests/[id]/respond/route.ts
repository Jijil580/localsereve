...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await db.collection("serviceRequests").findOneAndUpdate(
      ...
      { $push: { responses: { providerId: profile._id, providerName: session.fullName, providerBusiness: String(profile.businessName ?? session.fullName), providerWhatsApp: String(profile.phone ?? ""), message, createdAt: now } } as never, $set: { status: "quoted", updatedAt: now }, $inc: { quoteCount: 1 } },
