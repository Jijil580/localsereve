...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!ObjectId.isValid(id)) return Response.json({ error: "Invalid WhatsApp request" }, { status: 400 });
  ...
  try {
    ...
    const now = new Date();
    const result = await db.collection("whatsappContactRequests").findOneAndUpdate({ _id: new ObjectId(id), providerId: profile._id, status: "pending" }, { $set: { status, respondedAt: now, updatedAt: now } }, { returnDocument: "after" });
    if (!result) return Response.json({ error: "This request is no longer pending" }, { status: 409 });
    return Response.json({ ok: true, status });
  } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Unable to update WhatsApp request" }, { status: 500 }); }
}
