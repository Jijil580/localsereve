...
export async function GET() {
  ...
  try {
    ...
    const rows = await db.collection("whatsappContactRequests").find({ providerId: profile._id }).sort({ requestedAt: -1 }).limit(100).toArray();
  ...
  } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Unable to load WhatsApp requests" }, { status: 500 }); }
}
