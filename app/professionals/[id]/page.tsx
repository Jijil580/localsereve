    ...
    <main className="seo-page">
      ...
      <section className="public-profile-hero">
        <div className="public-profile-image">
          ...
          <div className="public-profile-banner-metrics" aria-label="Provider profile summary"><div><i>♥</i><span><b>{provider.likes}</b><small>{provider.likes===1?"Like":"Likes"}</small></span></div><div><i>★</i><span><b>{provider.reviews>0?provider.rating.toFixed(1):"New"}</b><small>{provider.reviews>0?`${provider.reviews} reviews`:"No ratings yet"}</small></span></div><div><i>↗</i><span><b>{provider.requestsReceived}</b><small>{provider.requestsReceived===1?"Request received":"Requests received"}</small></span></div></div>
        </div>
